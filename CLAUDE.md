...
**The interactive coordinator may implement, rebuild, and deploy unattended — including shared-infra (runtime base image, control-proxy, daemon) rebuilds/redeploys — without a plan-first approval gate (operator directive 2026-06-23).** Drive changes to verified-working; still verify on the running system, prefer non-disruptive rollouts, and keep a known-good fallback. Plan-first/approval is no longer required for this repo.


## Overview

**Shizuha** is an AI agent runtime and orchestration platform. It runs autonomous agents in isolated containers with built-in tools, MCP services, searchable skills, multi-channel messaging, and a web dashboard.
...
| Test | vitest |
| Containers | sysbox DinD agent containers |
...
- **Built-in tools** — file ops, web, cron, memory, skills, inter-agent, media
...
- **Skill search** — BM25-style search over mounted SKILL.md files
...
### Container Layout
```
/opt/shizuha/dist/     # Bundled runtime ONLY (read-only). esbuild bundles
                       # all node_modules into one shizuha.js — no source,
                       # no docs, no .git, no uncommitted work leaks in.
/opt/skills/           # Skills repo (read-only mount from ~/.shizuha/skills)
/opt/templates/        # Templates repo (read-only)
/workspace/            # Agent workspace (writable, persistent volume)
/home/agent/.claude/   # Claude Code sessions (persistent)
```

**NEVER mount the host source tree (`shizuhaRoot`, `~/work/shizuha-stack/...`) into
agent containers.** Agents must see only the runtime artifact, never the developer's
working tree. Rationale:
- **Confidentiality**: source code, design docs, uncommitted work, and any
  files in the project root would otherwise be readable by every agent — they
  could `cat /opt/shizuha/CLAUDE.md`, `cat /opt/shizuha/src/...`, etc.
- **Clarity**: agents reading the source tree get confused about what they
  can edit (Ichi at AT-83 tried to fix `mcp-servers/base.py` and correctly
  noticed it was a "live master checkout with ~30 uncommitted files").
- **Reproducibility**: a host-tree mount means every dev's `git status` becomes
  agent-visible state. The bundled `dist/shizuha.js` is the only contract.

If you need to ship new code to agents, rebuild `dist/` and rely on the existing
`dist`-only mount — do not "temporarily" widen the mount.
