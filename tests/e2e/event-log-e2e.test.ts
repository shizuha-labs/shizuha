...
const DASHBOARD_USER = process.env['DASHBOARD_USER'] || process.env['SHIZUHA_DASHBOARD_E2E_USERNAME'] || process.env['SHIZUHA_DASHBOARD_USERNAME'] || 'shizuha';
const DASHBOARD_PASSWORD = process.env['DASHBOARD_PASS'] || process.env['SHIZUHA_DASHBOARD_E2E_PASSWORD'] || process.env['SHIZUHA_DASHBOARD_PASSWORD'] || '';
const SAFE_HOSTS = new Set(['localhost', '127.0.0.1', '::1', 'shizuha-nginx', 'host.docker.internal']);
// The parallel unit suite starts ephemeral dashboard listeners on the default
// port. Never mistake one for an operator-authorized live E2E target: require
// an explicit target/credential or opt-in flag before sending agent messages.
const EVENT_LOG_E2E_ENABLED = process.env['RUN_EVENT_LOG_E2E'] === '1'
  || Boolean(process.env['DASHBOARD_URL'])
  || Boolean(process.env['SHIZUHA_DASHBOARD_E2E_PASSWORD']);

let sessionCookie = '';
let targetAgentId = '';
let targetAgentUsername = '';

function boundedFetch(url: string, init: RequestInit = {}, timeoutMs = 5_000): Promise<Response> {
...
async function login(): Promise<string> {
  const resp = await boundedFetch(`${DASHBOARD_URL}/v1/dashboard/login`, {
    method: 'POST',
...
async function getIsolatedAgent(cookie: string): Promise<{ id: string; username: string }> {
  const resp = await boundedFetch(`${DASHBOARD_URL}/v1/agents`, {
...
async function resetRuntimeSession(cookie: string, agentId: string): Promise<void> {
  await boundedFetch(`${DASHBOARD_URL}/v1/agents/${agentId}/reset-session`, {
    method: 'POST',
    headers: { Cookie: `shizuha_session=${cookie}` },
  }).catch(() => {});
}
...
describe('Event Log E2E', () => {
  beforeAll(async () => {
    if (!EVENT_LOG_E2E_ENABLED) {
      console.log('Event log live E2E not explicitly configured — skipping');
      return;
    }
    ...
    try {
      const health = await boundedFetch(`${DASHBOARD_URL}/health`);
