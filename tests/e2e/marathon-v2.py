...
def uid(): return hashlib.md5(str(time.time()).encode(), usedforsecurity=False).hexdigest()[:8]
...
def s_canvas(ag):
    ...
    txt, tools, ms = send(a["id"], f"Return a simple inline SVG for: {random.choice(shapes)}", 30)
    ok = bool(txt) or "svg" in str(tools).lower()
    return f"{a['name']} inline-svg", ok, ms, txt
