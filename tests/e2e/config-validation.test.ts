...
// Hermeticity (SCLI-11): registerBuiltinTools is env-conditional —
// AGENT_USERNAME+AGENT_PASSWORD add the Connect messaging tools and
// PLATFORM_PULSE_CONNECTED removes the local cron tools — so the registry
// assertions below silently depend on where the suite runs (40 tools on a
// bare CI runner, 42 inside an agent container). Pin the standalone
// no-credentials baseline for every test in this file.
const LEAKY_ENV = ['AGENT_USERNAME', 'AGENT_PASSWORD', 'PLATFORM_PULSE_CONNECTED'] as const;
let savedLeakyEnv: Record<string, string | undefined>;
beforeEach(() => {
  savedLeakyEnv = {};
  for (const k of LEAKY_ENV) {
    savedLeakyEnv[k] = process.env[k];
    delete process.env[k];
  }
});
afterEach(() => {
  for (const k of LEAKY_ENV) {
    if (savedLeakyEnv[k] === undefined) delete process.env[k];
    else process.env[k] = savedLeakyEnv[k];
  }
});
...
describe('Config Schema Validation', () => {
  it('parses empty object with all defaults applied', () => {
    ...
    expect(result.agent.maxOutputTokens).toBe(32768);
    ...
    expect(result.mcp.toolSearch.maxResults).toBe(3);
...
describe('Tool Registry Completeness', () => {
  ...
  const EXPECTED_TOOLS: Record<string, { riskLevel: 'low' | 'medium' | 'high'; readOnly: boolean }> = {
    ...
    remote_exec:       { riskLevel: 'high',   readOnly: false },
    ...
    // Inter-agent tools (message_agent/list_agents) are NOT builtin — direct
    // agent messaging goes through shizuha-connect, and list_agents is only
    // registered in gateway mode with agent context, not in the base registry.
    memory:            { riskLevel: 'low',    readOnly: false },
    memory_store:      { riskLevel: 'low',    readOnly: false },
    memory_search:     { riskLevel: 'low',    readOnly: true },
    memory_list:       { riskLevel: 'low',    readOnly: true },
    memory_forget:     { riskLevel: 'low',    readOnly: false },
    ...
    // Browser-input control tools (registered alongside the browser tool).
    keyboard:          { riskLevel: 'medium', readOnly: false },
    mouse:             { riskLevel: 'medium', readOnly: false },
