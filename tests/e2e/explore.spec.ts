...
const USER = process.env['DASHBOARD_USER'] || process.env['SHIZUHA_DASHBOARD_E2E_USERNAME'] || process.env['SHIZUHA_DASHBOARD_USERNAME'] || 'shizuha';
const PASS = process.env['DASHBOARD_PASS'] || process.env['SHIZUHA_DASHBOARD_E2E_PASSWORD'] || process.env['SHIZUHA_DASHBOARD_PASSWORD'] || '';
...
const PROMPTS = [
  ...
  'Return a simple inline HTML counter app',
  'memory(action="list")',
  ...
  'Generate a simple inline SVG circle',
...
test.describe('Exploration Session', () => {
  ...
  test('E9: Inline SVG render', async ({ page }) => {
    test.setTimeout(120000);
    await login(page);
    await selectAgent(page, 'Sora');
    await sendMessage(page, 'Return a simple inline SVG: a blue circle with radius 40 on a white background, 200x200');
    await waitForResponse(page);
    await screenshot(page, 'e9-inline-svg');
