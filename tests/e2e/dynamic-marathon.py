...
def scenario_canvas(agents):
    """Ask agent to return inline SVG."""
    a = random.choice(agents)
    shapes = [
    ...
    r = send_ws(a["id"], f'Return a simple inline SVG: {random.choice(shapes)}', timeout=30)
    ok = r["has_response"]
    log(f"  inline-svg({a['name']}): {'PASS' if ok else 'FAIL'} ({r['ms']}ms)")
    return ok
