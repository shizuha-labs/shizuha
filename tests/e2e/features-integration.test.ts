...
describe('8. Config Schema Integration', () => {
  it('parses empty object with defaults', async () => {
    ...
    expect(config.agent.maxOutputTokens).toBe(32768);
...
// ════════════════════════════════════════════════════════════════════════════
// Cross-feature: Transitive URI resolver compatibility
// ════════════════════════════════════════════════════════════════════════════

describe('Cross-feature: Ajv URI resolver compatibility', () => {
  it.each([
    [
    ...
    [
      ...
      'http://trusted.com%40evil.com/schema',
      'http://trusted.com@evil.com/schema',
    ],
  ])('keeps %s distinct at the Ajv consumer boundary', async (_case, encodedId, canonicalId) => {
    const { default: Ajv } = await import('ajv');
    const ajv = new Ajv({ strict: false });

    ajv.addSchema({ $id: encodedId, type: 'string', const: 'encoded' });
    ajv.addSchema({ $id: canonicalId, type: 'string', const: 'canonical' });

    expect(ajv.validate(encodedId, 'encoded')).toBe(true);
    expect(ajv.validate(encodedId, 'canonical')).toBe(false);
    expect(ajv.validate(canonicalId, 'canonical')).toBe(true);
    expect(ajv.validate(canonicalId, 'encoded')).toBe(false);
  });
});
