  ...
  describe('schedule-based repeat behavior', () => {
    ...
    it('wakeup loops can carry explicit iteration caps and metadata', async () => {
      const job = await store.addJob({
        name: 'Monitor loop',
        prompt: 'check status',
        schedule: parseSchedule('every 30s'),
        deliver: delivery(),
        kind: 'wakeup',
        repeatTimes: 3,
        loop: {
      ...
      expect(job.repeat.times).toBe(3);
      expect(job.loop?.key).toBe('monitor-loop');
      expect(store.getLoopStats('monitor-loop')).toMatchObject({ jobs: 1, active: 1, completed: 0 });
    });
  });

  describe('wakeup loop guardrails', () => {
    it('prevents duplicate active dynamic wakeups and enforces max iterations', async () => {
      const loop = { key: 'dynamic-monitor', mode: 'dynamic' as const, maxIterations: 2, cooldownMs: 0 };
      const first = await store.addJob({
        name: 'Dynamic monitor',
        prompt: 'iteration 1',
        schedule: parseSchedule('1s'),
        deliver: delivery(),
        kind: 'wakeup',
        repeatTimes: 1,
        loop,
      ...
      await expect(store.addJob({
        ...
        repeatTimes: 1,
        loop,
      })).rejects.toThrow('already has an active scheduled wakeup');

      await store.markJobRun(first.id, 'ok');
      await store.addJob({
        name: 'Dynamic monitor',
        prompt: 'iteration 2',
        schedule: parseSchedule('1s'),
      ...
      await expect(store.addJob({
        ...
        repeatTimes: 1,
        loop,
      })).rejects.toThrow('has reached maxIterations');
    });

    it('claimJob prevents duplicate due-job enqueue until completion', async () => {
      const job = await store.addJob({
        name: 'Due once',
        prompt: 'run',
        schedule: parseSchedule('1s'),
      ...
      await expect(store.claimJob(job.id)).resolves.toBe(true);
      expect(store.getJob(job.id)?.running).toBe(true);
      expect(store.getDueJobs()).toHaveLength(0);

      await store.markJobRun(job.id, 'ok');
      expect(store.getJob(job.id)?.running).toBe(false);
...
describe('CronScheduler', () => {
  ...
  it('tick submits wakeup jobs as self-invocations to the original thread', async () => {
    const job = await store.addJob({
      name: 'Wakeup Test',
      ...
      kind: 'wakeup',
      repeatTimes: 3,
      loop: { key: 'wakeup-test', mode: 'fixed', maxIterations: 3, cooldownMs: 0 },
    });
    vi.spyOn(store, 'getDueJobs').mockReturnValue([job]);
    ...
    expect(submitToInbox).toHaveBeenCalledTimes(1);
    const msg = submitToInbox.mock.calls[0]![0];
    expect(msg.content).toBe('Continue the loop');
    expect(msg.source).toBe('cron');
    expect(msg.threadId).toBe('th-1');
    expect(msg.userName).toBe('ScheduleWakeup');
    expect(msg.metadata).toMatchObject({ selfInvocation: true });
  });
