...
describe('handleSlashCommand', () => {
  ...
  describe('/help', () => {
    ...
    it('supports full help variant', () => {
      ...
      expect(result.handled).toBe(true);
      // SCLI-383: full help opens the pager (q/Esc dismiss) — no status-line
      // message that left the draft active under "any key".
      expect(result.message).toBeUndefined();
      expect(ctx.showInPager).toHaveBeenCalledTimes(1);
      const body = (ctx.showInPager as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as string;
      expect(body).toContain('[Session]');
      expect(body).toContain('/config [subcommand]');
      expect(body).toContain('/help all');
    });

    it('falls back to message when pager is unavailable', () => {
  ...
  describe('/watch', () => {
    it('registers a background-task watch with notify policy', () => {
      const registry = new BackgroundTaskRegistry();
      const task = registry.create('bash', 'deploy rollout');
      const onWatchesChanged = vi.fn();
      const ctx = createMockContext({ getTaskRegistry: () => registry, onWatchesChanged });

      const result = handleSlashCommand('/watch deploy --policy state_changes', ctx);

      expect(result.handled).toBe(true);
      expect(result.message).toContain('Watching deploy');
      expect(registry.listWatches()).toHaveLength(1);
      expect(registry.get(task.id)?.notifyPolicy).toBe('state_changes');
      expect(onWatchesChanged).toHaveBeenCalled();
    });

    it('lists and removes watches', () => {
      const registry = new BackgroundTaskRegistry();
      registry.watch('benchmark', 'done_only');
      const ctx = createMockContext({ getTaskRegistry: () => registry, onWatchesChanged: vi.fn() });

      const listed = handleSlashCommand('/watch list', ctx);
      expect(listed.message).toContain('benchmark');

      const removed = handleSlashCommand('/watch remove benchmark', ctx);
      expect(removed.message).toContain('Removed watch');
      expect(registry.listWatches()).toHaveLength(0);
    });
  });
  ...
  describe('/rename', () => {
    it('requires a name argument', () => {
      const ctx = createMockContext({ renameSession: vi.fn(() => true) });
      const result = handleSlashCommand('/rename', ctx);
      expect(result.handled).toBe(true);
      expect(result.message).toContain('Usage');
    });

    it('renames with argument when a durable session exists', () => {
      const renameSession = vi.fn(() => true);
      const ctx = createMockContext({ renameSession });
      const result = handleSlashCommand('/rename my-session', ctx);
      expect(result.handled).toBe(true);
      expect(renameSession).toHaveBeenCalledWith('my-session');
  ...
  describe('/init', () => {
    ...
    it('does not overwrite existing AGENTS.md', () => {
      const sentinel = 'existing content — do not touch\n';
      fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), sentinel);
      const ctx = createMockContext({ cwd: tmpDir });
      const result = handleSlashCommand('/init', ctx);
      expect(result.handled).toBe(true);
      expect(result.message).toContain('already exists');
      // SCLI-391: existing file must remain byte-for-byte unchanged
      expect(fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8')).toBe(sentinel);
    });

    it('SCLI-391: Python-only fixture emits Python commands, not npm', () => {
      ...
      expect(body).toMatch(/pytest|uv run/);
      expect(body).not.toMatch(/\bnpm\b/);
    });

    it('SCLI-391: Node fixture may emit npm commands', () => {
      fs.writeFileSync(
        path.join(tmpDir, 'package.json'),
        JSON.stringify({ name: 'demo', scripts: { build: 'tsc', test: 'vitest', dev: 'vite' } }),
      );
      const ctx = createMockContext({ cwd: tmpDir });
      const result = handleSlashCommand('/init', ctx);
      expect(result.handled).toBe(true);
    ...
    it('SCLI-391: unknown fixture does not assert a specific ecosystem', () => {
      ...
      expect(body).not.toMatch(/\bnpm\b/);
      expect(body).not.toMatch(/\bpytest\b/);
      expect(body).not.toMatch(/\bcargo\b/);
    });
  });
  ...
  describe('/rename (SCLI-390)', () => {
    it('reports success only when renameSession returns true', () => {
      const renameSession = vi.fn(() => true);
      const ctx = createMockContext({ renameSession });
      const result = handleSlashCommand('/rename durable-name', ctx);
      expect(result.handled).toBe(true);
      expect(renameSession).toHaveBeenCalledWith('durable-name');
      expect(result.message).toBe('Session renamed to "durable-name"');
    });

    it('rejects with no success wording when there is no active session', () => {
      // Fresh zero-turn TUI: renameSession returns false (no sessionId).
      const renameSession = vi.fn(() => false);
      const ctx = createMockContext({ renameSession });
      const result = handleSlashCommand('/rename SCLI178_FALSE_SUCCESS_CONTROL', ctx);
      expect(result.handled).toBe(true);
      expect(renameSession).toHaveBeenCalledWith('SCLI178_FALSE_SUCCESS_CONTROL');
      expect(result.message).toBe('No active session to rename');
      expect(result.message).not.toMatch(/renamed/i);
    });
    ...
    it('requires a name argument', () => {
      ...
      const result = handleSlashCommand('/rename', ctx);
      expect(result.handled).toBe(true);
      expect(result.message).toContain('Usage: /rename');
      expect(renameSession).not.toHaveBeenCalled();
    });

    it('returns not available when renameSession is not provided', () => {
      const ctx = createMockContext();
      const result = handleSlashCommand('/rename orphan', ctx);
      expect(result.handled).toBe(true);
      expect(result.message).toContain('not available');
    });
  });
    ...
    it('shows warning when usage is high', () => {
      const ctx = createMockContext({
        getSessionInfo: vi.fn(() => ({
          ...
          contextTokens: 110000, startTime: Date.now(), cwd: '/tmp',
        })),
      });
      const result = handleSlashCommand('/context', ctx);
      expect(result.handled).toBe(true);
  ...
  describe('/doctor', () => {
    it('runs full doctor diagnostics via async path (SCLI-387)', async () => {
      // Never hit a live Cortex catalog from unit tests (alert #25183).
      ...
      try {
        ...
        const result = await handleSlashCommandAsync('/doctor', ctx);
        expect(result.handled).toBe(true);
        expect(result.message).toContain('shizuha doctor');
        expect(result.message).toContain('Node.js version');
        expect(result.message).toContain('Selected model');
        expect(result.message).toMatch(/Results:/);
      } finally {
  ...
  describe('handleSlashCommandAsync auth', () => {
    it('handles /login success', async () => {
      ...
      expect(result.message).toContain('Provider auth refreshed');
      expect(result.message).toContain('MCP auth reloaded');
    });

    it('handles /login success when MCP reload is delayed', async () => {
      const ctx = createMockContext({
        loginShizuha: vi.fn(async () => ({
          username: 'sara',
          providerReloaded: true,
          mcpReloaded: false,
          reloadError: 'MCP auth reload timed out after 5000ms',
        })),
      });
      const result = await handleSlashCommandAsync('/login sara secret123', ctx);
      expect(result.handled).toBe(true);
      expect(result.message).toContain('Logged in as sara');
      expect(result.message).toContain('Provider auth refreshed');
      expect(result.message).toContain('MCP auth reload pending');
