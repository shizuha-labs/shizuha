...
describe('Memory Tool', () => {
  ...
  describe('compatibility native tools', () => {
    it('stores, searches, lists, and forgets through native memory_* wrappers', async () => {
      const storeResult = await memoryStoreTool.execute({
        content: 'Native memory wrapper works',
        category: 'facts',
        tags: ['scli'],
      }, ctx());
      expect(storeResult.isError).toBeUndefined();
      expect(storeResult.content).toContain('Added');

      const searchResult = await memorySearchTool.execute({ query: 'wrapper' }, ctx());
      expect(searchResult.isError).toBeUndefined();
      expect(searchResult.content).toContain('Native memory wrapper works');

      const listResult = await memoryListTool.execute({}, ctx());
      expect(listResult.isError).toBeUndefined();
      expect(listResult.content).toContain('[facts] Native memory wrapper works #scli');

      const forgetResult = await memoryForgetTool.execute({ id: 'Native memory wrapper works' }, ctx());
      expect(forgetResult.isError).toBeUndefined();
      expect(forgetResult.content).toContain('Removed');
    });
  });
