...
describe('write tool — append mode (2026-08-09 chunked large files)', () => {
  it('appends to an existing file instead of overwriting', async () => {
    const fp = path.join(tmpDir, 'big.py');
    await writeTool.execute({ file_path: fp, content: 'chunk1\n' }, ctx);
    const res = await writeTool.execute({ file_path: fp, content: 'chunk2\n', append: true }, ctx);
    expect(res.isError).toBeFalsy();
    expect(res.content).toContain('Appended to');
    expect(await fs.readFile(fp, 'utf-8')).toBe('chunk1\nchunk2\n');
  });

  it('append on a missing file just creates it', async () => {
    const fp = path.join(tmpDir, 'fresh.py');
    const res = await writeTool.execute({ file_path: fp, content: 'first\n', append: true }, ctx);
    expect(res.isError).toBeFalsy();
    expect(res.content).toContain('Created new file');
    expect(await fs.readFile(fp, 'utf-8')).toBe('first\n');
  });

  it('default remains full overwrite for backward compatibility', async () => {
    const fp = path.join(tmpDir, 'plain.txt');
    await writeTool.execute({ file_path: fp, content: 'old\n' }, ctx);
    const res = await writeTool.execute({ file_path: fp, content: 'new\n' }, ctx);
    expect(res.content).toContain('Overwrote');
