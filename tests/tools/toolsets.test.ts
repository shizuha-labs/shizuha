...
describe('Role-based profiles', () => {
  ...
  it('role profiles enforce MCP access matrix from CLAUDE.md', () => {
    ...
    // Security: pulse, wiki, drive, id — NOT admin, notes, mail, and NOT the
    // decommissioned SCS service (removed from the platform 2026-07-02).
    ...
    expect(sec).not.toContain('mcp__scs__list_instances');
    expect(sec).not.toContain('mcp__admin__list_users');
    expect(sec).not.toContain('mcp__mail__send_email');

    // Data: pulse, wiki, drive, books, finance, inventory. HR/time are retired platform MCPs.
    ...
    expect(data).not.toContain('mcp__hr__list_employees');
    expect(data).not.toContain('mcp__time__list_entries');
