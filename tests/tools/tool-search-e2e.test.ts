...
describe('Tool Search E2E — full pipeline simulation', () => {
  ...
  it('discovered tools are callable through the registry', async () => {
    ...
    // Select a known no-required-args tool explicitly. The default search cap is
    // intentionally small (3), so a broad billing query need not rank this one
    // in its first page.
    const toolName = 'mcp__mock-multi-service__billing_list_invoices';
    await toolRegistry.get('ToolSearch')!.execute(
      { query: `select:${toolName}` },
      { cwd: '/tmp', sessionId: 'test' },
    );
    expect(toolSearchState.getDiscovered().has(toolName)).toBe(true);
