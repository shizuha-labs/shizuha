...
describe('agent auth against dashboard gateway', () => {
  ...
  // Hermeticity (SCLI-11): AGENT_USERNAME leaking from an agent container makes
  // loadOrCreateAgentKeypair write fixture keypairs under the HOST agent's
  // identity dir instead of the fixture agent's — clear for the whole suite
  // (also keeps the spawned dashboard child env clean via {...process.env}).
...
(async () => {
  ...
  it('allows agent-scoped bearer auth on gateway endpoints', async () => {
    ...
    const askResp = await fetch(`${baseUrl}/v1/agents/${target.username}/message`, {
      method: 'POST',
      headers: {
