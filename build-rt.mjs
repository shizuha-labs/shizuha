 ...
 *   node build-rt.mjs --out ../../rt/dist/shizuha.min.js # External runtime repo
 *
...
await build({
  ...
  banner: {
    js: [
      ...
      '(()=>{',
      'const p=process.env["SHIZUHA_PRIVATE_ENV_JSON"];',
      'if(!p)return;',
      'try{',
      ...
      'if(typeof k==="string"&&k&&!/[=\\u0000\\r\\n]/.test(k)&&typeof v==="string"){process.env[k]=v;}',
      '}',
      '}catch{}',
      '})();',
    ].join('\n'),
...
if (!code.includes('SHIZUHA_PRIVATE_ENV_JSON')) {
  throw new Error('Runtime bundle missing private env JSON bootstrap');
}
...
// ── Copy daemon templates alongside the bundle ──────────────────────────
// The runtime resolver (src/daemon/heartbeat-template.ts) probes
// <bundle-dir>/templates and <bundle-dir>/../templates. Emit them next to the
// output bundle so both the dev-bundle (dist/templates) and the release-tarball
// (build-dist.sh stages these into lib/templates) layouts can resolve them.
// This is the runtime build entrypoint build-dist.sh actually invokes —
// esbuild.config.js does the same copy for the esbuild path, but the shipped
// tarball is assembled from what build-rt.mjs produces.
console.log('Post-build: Copy daemon templates...');
const templatesSrc = path.join('src', 'daemon', 'templates');
const templatesDst = path.join(path.dirname(outFile), 'templates');
if (fs.existsSync(templatesSrc)) {
  ...
  console.log(`   Copied ${templateCount} daemon template(s) to ${templatesDst}\n`);
} else {
  console.log(`   No daemon templates at ${templatesSrc} — skipping\n`);
}
