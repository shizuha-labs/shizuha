...
const MODEL_CONTEXT: Record<string, number> = {
  // 4.x / 5.x family (short slugs)
  'claude-fable-5': 1_000_000,
  'claude-opus-5': 1_000_000,
  'claude-opus-4-8': 1_000_000,
  'claude-opus-4-7': 1_000_000,
  'claude-opus-4-6': 1_000_000,
  'claude-sonnet-4-6': 1_000_000, // Sonnet 4.x supports 1M via long-context beta (operator 2026-06-24)
...
export class AnthropicProvider implements LLMProvider {
   ...
   *  so pre-chat budget/compaction uses the right window (e.g. opus-4-8 / fable-5
   *  = 1M, not the 200K class default). */
  ...
  async *chat(messages: ChatMessage[], options: ChatOptions): AsyncGenerator<StreamChunk> {
    ...
    this.maxContextWindow = this.contextWindowFor(options.model);
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        // Models that have removed sampling parameters (temperature/top_p/top_k).
        // Sending any of these returns 400 on the API.
        const noSamplingParams = /^claude-(opus-4-[789]|opus-5|sonnet-5|fable-5)\b/.test(options.model);

        // Build extended thinking params if thinking is enabled.
        // On models that removed sampling params, use adaptive thinking (never send
        // {type: "enabled", budget_tokens} — that returns 400 on 4.7/4.8/fable-5).
        const thinkingParams: Record<string, unknown> = {};
        if (options.thinkingLevel && options.thinkingLevel !== 'off') {
          if (noSamplingParams) {
            // Adaptive: API decides depth. Omitting the param entirely is equivalent
            // on Fable 5 and Opus 5; on Opus 4.7/4.8 the adaptive type is required
            // (omitting it there means no thinking at all).
            ...
            // Extended thinking requires temperature = 1 for Anthropic (pre-4.7 models)
            thinkingParams['temperature'] = 1;
          }
        }

        // Opus 4.6–4.9 and Sonnet 4.x unlock 1M-token context via the long-context
        // beta header. Without it the API caps inputs around 200K.
        // Fable 5, Opus 5 and Sonnet 5 have 1M as their default — no header needed.
        const longContextModel = /^claude-(opus-4-[6-9]|sonnet-4)\b/.test(options.model);
        const extraHeaders = longContextModel
        ...
        const stream = this.client.messages.stream(
          {
            ...
            // Models without sampling params (Opus 4.7/4.8/Fable 5) 400 on temperature.
            // Legacy models: temperature=1 when thinking enabled, else use the passed value.
            ...(noSamplingParams
              ? {}
              : thinkingParams['temperature'] != null
                ? { temperature: thinkingParams['temperature'] as number }
          ...
          { signal: abortController.signal, ...(extraHeaders ? { headers: extraHeaders } : {}) },
        );
        ...
        if (abortController.signal.aborted) {
          ...
          // Signal the SCLI-22 watchdog stall before the final retry guard too;
          // otherwise a last-attempt first-token/stream timeout fails the turn
          // without emitting the STALL signal StruggleAnalyzer consumes.
          yield {
            type: 'status' as const,
            code: 'stall_timeout',
            message: `No response for ${Math.round((gotFirstEvent ? STREAM_TIMEOUT_MS : FIRST_TOKEN_TIMEOUT_MS) / 1000)}s — ${willRetry ? `retrying (${attempt + 1}/${MAX_RETRIES + 1})` : 'stall'}`,
            level: 'warning' as const,
            provider: 'anthropic',
            attempt: attempt + 1,
            maxAttempts: MAX_RETRIES + 1,
            retryInMs: willRetry ? delay : undefined,
          };
