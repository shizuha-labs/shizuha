...
export class CopilotProvider implements LLMProvider {
  ...
  async *chat(messages: ChatMessage[], options: ChatOptions): AsyncGenerator<StreamChunk> {
    // Strip provider prefix if present (e.g. "copilot/claude-opus-4.6" → "claude-opus-4.6")
    if (options.model.includes('/')) {
      options = { ...options, model: options.model.split('/').pop()! };
    }
    ...
    // Inject cache_control for Claude models — Copilot may forward to Anthropic
    if (options.model.startsWith('claude') && openaiMessages.length > 0 && openaiMessages[0]!.role === 'system') {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        logger.info({ requestedModel: options.model }, 'Copilot API call');
