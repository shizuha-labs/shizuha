...
function normalizeCodexModel(model: string): string {
  if (model.includes('codex-spark')) return model;
  if (/^gpt-5\.(?:3|4)(?:$|-)/.test(model)) return CANONICAL_CODEX_MODEL;
  return model;
}

/** Context windows — sourced from codex-rs/core/models.json */
const MODEL_CONTEXT: Record<string, number> = {
  [CANONICAL_CODEX_MODEL]: 272000,
...
function normalizeReasoningEffort(effort: string | null | undefined): SupportedReasoningEffort {
  ...
  // that the direct Responses API does not accept — clamp to the highest it
  // does. The codex-bridge (app-server → chatgpt.com/backend-api) forwards the
  // raw effort string instead, so ultra reaches the backend intact on that path.
  if (level === 'xhigh' || level === 'ultra' || level === 'max') return 'xhigh';
 ...
 */
/** SCLI-101: Fetch the current Codex access token from the daemon broker endpoint */
async function fetchBrokerToken(brokerUrl: string): Promise<string | null> {
  try {
    const resp = await fetch(brokerUrl, { signal: AbortSignal.timeout(5000) });
    if (!resp.ok) {
      logger.warn({ brokerUrl, status: resp.status }, '[codex-broker] token fetch failed');
      return null;
    }
    const data = await resp.json() as Record<string, unknown>;
    const token = data.accessToken as string | undefined;
    if (!token) {
      logger.warn({ brokerUrl }, '[codex-broker] response missing accessToken');
      return null;
    }
    return token;
  } catch (err) {
    logger.warn({ brokerUrl, err: (err as Error).message }, '[codex-broker] fetch error');
    return null;
  }
}
...
function readAllCodexAccounts(): CodexAuth[] {
  ...
  // their own OAuth refresh — the daemon holds the single refresh_token.
  const codexBrokerUrl = process.env['CODEX_BROKER_URL'];
  if (codexBrokerUrl) {
    logger.info({ brokerUrl: codexBrokerUrl }, 'Using CODEX_BROKER_URL env var (broker mode)');
    return [{
      authMode: 'chatgpt',
      email: 'broker',
...
export class CodexProvider implements LLMProvider {
  ...
  async refreshExpiredTokens(): Promise<void> {
    ...
    for (let i = 0; i < this.pool.size; i++) {
      ...
      if (!account) continue;
      // Broker mode (SCLI-101): always fetch a fresh token from the daemon broker
      if (account.email === 'broker') {
        const brokerUrl = process.env['CODEX_BROKER_URL'];
        if (brokerUrl) {
          const newToken = await fetchBrokerToken(brokerUrl);
          if (newToken) {
            account.auth.accessToken = newToken;
            this.pool.refreshClient(i);
            logger.info('[codex-broker] proactive token fetch OK');
          } else {
  ...
  async *chat(messages: ChatMessage[], options: ChatOptions): AsyncGenerator<StreamChunk> {
    ...
    if (normalizedModel !== options.model) {
      logger.warn({ fromModel: options.model, toModel: normalizedModel }, 'Codex model upgraded to canonical default');
    }
    this.maxContextWindow = MODEL_CONTEXT[normalizedModel] ?? 272000;
    ...
    for (let attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (requestAbort.signal.aborted) {
          ...
          // Emit SCLI-22 watchdog signal so StruggleAnalyzer detects STALL without
          // needing its own idle timer. Fire once per stall attempt (before retry/throw).
          const timeout = gotFirstEvent ? STREAM_TIMEOUT_MS : FIRST_TOKEN_TIMEOUT_MS;
          yield {
            type: 'status' as const,
            code: 'stall_timeout',
            message: `No response for ${Math.round(timeout / 1000)}s — ${attempt < MAX_RETRY_ATTEMPTS ? `retrying (${attempt + 1}/${MAX_RETRY_ATTEMPTS + 1})` : 'stall'}`,
        ...
        if (err.status === 401) {
          ...
          // Broker mode (SCLI-101): re-fetch from daemon broker instead of local refresh
          if (account.email === 'broker') {
            const brokerUrl = process.env['CODEX_BROKER_URL'];
            if (brokerUrl) {
              const brokerToken = await fetchBrokerToken(brokerUrl);
              if (brokerToken) {
                account.auth.accessToken = brokerToken;
                this.pool.refreshClient(this.pool.currentIndex);
                logger.info('[codex-broker] 401 recovery: fetched fresh token from broker');
                continue;
              }
            }
        ...
        // Benchmark comparisons must preserve explicit Spark model selection.
        // If direct ChatGPT/Codex rejects a *-spark model, fail closed instead
        // of silently retrying the non-Spark sibling.
        if (err.status === 400 && requestModel.endsWith('-spark')) {
          throw new Error(
            `Codex Spark model "${requestModel}" was rejected by the direct Codex backend; ` +
            `route benchmark Spark aliases through Cortex (CTX-400) or pick an explicit non-Spark model. ${err.message}`,
          );
        }
  ...
  private async createStream(
    ...
    if (caps.supportsReasoningSummaries && !disableReasoningPayload) {
      ...
      // Use 'auto' so the user can see thinking output. Codex defaults to hiding
      // summaries for some models, but shizuha streams reasoning text to the TUI/CLI.
      (params as unknown as Record<string, unknown>).reasoning = { effort, summary: 'auto' };
