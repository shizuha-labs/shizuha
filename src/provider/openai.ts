...
export class OpenAIProvider implements LLMProvider {
  ...
  /** When true, inject cache_control for Claude models (LiteLLM proxy) */
  ...
  /** Per-model native context window. Subclasses (xAI/Groq/Together) override
   *  with their own tables; the base uses the OpenAI MODEL_CONTEXT map.
   *  SCLI-81: public + on the LLMProvider interface so pre-chat budget/compaction
   *  resolves the right window without a chat() call. */
  contextWindowFor(model: string): number {
    return MODEL_CONTEXT[model] ?? 128000;
  }

  /** Parse accumulated streamed tool-call arguments into an object. Subclasses
   *  override to handle provider-specific quirks (e.g. Groq fenced/double-encoded
   *  JSON). Must never throw — return {} on any malformed input. */
  protected parseToolCallArguments(raw: string): Record<string, unknown> {
    try {
  ...
  async *chat(messages: ChatMessage[], options: ChatOptions): AsyncGenerator<StreamChunk> {
    ...
    // Inject cache_control for Claude models via LiteLLM proxy
    // LiteLLM translates these to Anthropic's native cache_control format
    const isClaudeModel = options.model.startsWith('claude') || options.model.includes('claude');
    if (this.enablePromptCaching && isClaudeModel) {
      // Cache the system prompt (largest, most stable content)
      if (openaiMessages.length > 0 && openaiMessages[0]!.role === 'system') {
        const sys = openaiMessages[0] as any;
        if (typeof sys.content === 'string') {
          sys.content = [{ type: 'text', text: sys.content, cache_control: { type: 'ephemeral' } }];
        }
      }
      // Cache tool definitions (stable across turns)
      if (tools?.length) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const stream = await this.client.chat.completions.create({
          ...
          // Cortex pins the session to its prefix/KV-warm replica.
        ...
        for await (const chunk of stream) {
          ...
          if (choice.finish_reason) {
            ...
            for (const [, tc] of toolCalls) {
              const input = this.parseToolCallArguments(tc.args);
