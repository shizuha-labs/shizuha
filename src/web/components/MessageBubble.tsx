...
import { cacheWorkspaceImages, resolveHtmlBlobUrls } from '../lib/image-cache';
...
export const MessageBubble = memo(function MessageBubble({ message }: MessageBubbleProps) {
  ...
  // Cache workspace images as blob URLs (WhatsApp-style — fetch once, never re-fetch).
  // Only runs when message content changes (React.memo prevents scroll-triggered re-renders).
  useEffect(() => {
    cacheWorkspaceImages(markdownRef.current);
  }, [message.content]);
  ...
  if (isUser) {
    const isFromAgent = message.source?.startsWith('agent:');
    const senderName = isFromAgent ? message.source!.replace('agent:', '@') : null;
    const isHeartbeat = message.content?.startsWith('[Heartbeat]');
    const isScheduledTask = message.content?.startsWith('[Scheduled Task') || message.content?.startsWith('[Task Assigned]') || message.content?.startsWith('[Platform Task');

    return (
      <div className={`flex ${isFromAgent || isHeartbeat || isScheduledTask ? 'justify-start' : 'justify-end'} mb-3 relative group/msg`}>
        <div className={`max-w-[75%] min-w-[60px] relative`}>
          {/* Sender label for inter-agent and system messages */}
          {senderName && (
            <div className="flex items-center gap-1.5 mb-1 ml-1">
              <span className="text-[10px] font-semibold text-violet-400">{senderName}</span>
              <span className="text-[9px] text-zinc-600">agent message</span>
            </div>
          )}
          {isHeartbeat && !senderName && (
            <div className="flex items-center gap-1.5 mb-1 ml-1">
              <span className="text-[9px] text-amber-600">heartbeat</span>
            </div>
          )}
          {isScheduledTask && !senderName && (
            <div className="flex items-center gap-1.5 mb-1 ml-1">
              <span className="text-[9px] text-cyan-600">system</span>
            </div>
          )}
          {/* Attached images */}
          ...
          <div className={`${
            isFromAgent ? 'bg-violet-900/30 border border-violet-800/30' :
            isHeartbeat ? 'bg-amber-900/20 border border-amber-800/20' :
            isScheduledTask ? 'bg-cyan-900/20 border border-cyan-800/20' :
            'bg-shizuha-700'
          } text-zinc-100 rounded-2xl ${isFromAgent || isHeartbeat || isScheduledTask ? 'rounded-tl-sm' : 'rounded-tr-sm'} px-4 py-2.5 shadow-sm`}>
            <div
              className="markdown-content text-sm text-zinc-100 leading-relaxed select-text"
              dangerouslySetInnerHTML={{ __html: renderMarkdown(message.content) }}
            />
          </div>
          {/* Metadata row with info button — below bubble, not overlaying text */}
          <div className="flex items-center justify-end gap-1.5 mt-0.5 pr-1">
            ...
            <button
              ...
              className="text-[10px] text-zinc-700 hover:text-zinc-400 transition-colors cursor-pointer opacity-0 group-hover/msg:opacity-100 sm:opacity-100 sm:text-zinc-700"
              title="Message info"
            >
              i
            </button>
  ...
  // Assistant message
  return (
    <div className="flex justify-start mb-3 relative group/msg">
      <div className="flex gap-2.5 w-full">
        ...
        <div className="min-w-0 flex-1">
          ...
          {/*
            Tool calls intentionally NOT rendered in the chat panel.
            The dashboard chat is meant to mirror what the user sees on
            mobile/web Connect — only the agent's `message_user` reply.
            Internal tool mechanics (which MCP calls fired, in what order,
            with what errors) belong in the Activity tab, never inline
            with chat bubbles. The full tool trace is still available there.
            Long-press the bubble for an info overlay if you need a quick
            "this message used N tools" hint.
          */}
            ...
            isInteractiveContent(message.content) ? (
              ...
              <div className="max-w-[85%]">
                <div className={`rounded-2xl rounded-tl-sm px-4 py-2.5 shadow-sm ${
                  ...
                  <div
                    ref={markdownRef}
                    className="markdown-content text-sm text-zinc-200 leading-relaxed select-text"
                    dangerouslySetInnerHTML={{ __html: resolveHtmlBlobUrls(renderMarkdown(message.content)) }}
                  />
                </div>
              </div>
            )
          ...
          <div className="mt-0.5 pl-1 flex items-center gap-2">
            ...
            <button
              onClick={() => setShowInfo(true)}
              className="text-[10px] text-zinc-700 hover:text-zinc-400 transition-colors cursor-pointer sm:opacity-100 opacity-0 group-hover/msg:opacity-100 ml-auto"
              title="Message info"
            >
              info
            </button>
