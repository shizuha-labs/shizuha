...
function getEffortValues(method?: string, model?: string): string[] {
  ...
  // Gemini CLI / OpenClaw don't support effort
  if (method === 'gemini_cli_server' || method === 'openclaw_bridge') return [];
  // shizuha/direct — model-dependent
  if (model?.startsWith('claude-') || model?.includes('/claude-')) return ['low', 'medium', 'high', 'max'];
  ...
  // vLLM, Ollama, Gemini, and other models don't support effort
  if (model?.startsWith('vllm/') || model?.startsWith('ollama/') || model?.startsWith('gemini')) return [];
  return []; // Don't show for unknown models
}
...
function getThinkingValues(method?: string, model?: string): string[] {
  ...
  // Gemini CLI / OpenClaw don't expose thinking controls
  if (method === 'gemini_cli_server' || method === 'openclaw_bridge') return [];
  // shizuha/direct — model-dependent
  if (model?.startsWith('claude-') || model?.includes('/claude-')) return ['off', 'on', 'low', 'medium', 'high'];
  if (model?.startsWith('gpt-5') || model?.startsWith('codex-')) return [];
  // vLLM models with thinking support get a simple on/off toggle
  if (model?.startsWith('vllm/')) return ['off', 'on'];
  // Ollama, Gemini — no thinking control
  if (model?.startsWith('ollama/') || model?.startsWith('gemini')) return [];
  return []; // Don't show for unknown models
}
...
export function StatusBar({
  ...
  const availableModels = useDashboardModels([model]);
          ...
          {thinkingValues.length > 0 && (
            <div className="relative">
              <button
                ...
                title={`Thinking: ${thinking || 'off'}. Click to change.`}
              >
