...
export default function App() {
  ...
  const [selectedAgent, setSelectedAgent] = useState<Agent | null>(() => {
    ...
    try {
      ...
      const agent = list.find((a) => a.id === savedId) ?? null;
      // Set markdown context for workspace image paths
      if (agent) {
        import('./lib/markdown.js').then((m) => m.setMarkdownAgentContext(agent.username));
      }
      return agent;
    } catch { return null; }
  });
  ...
  const handleSyncEvent = useCallback((event: { type: string; [key: string]: unknown }) => {
    if (event.type === 'sync_pending_update') {
      setSyncPending(prev => [...prev, event.update as any]);
    } else if (event.type === 'sync_conflicts') {
      setSyncConflicts(event.conflicts as any[]);
    }
  }, []);
  ...
  const tryLogin = useCallback(async (username: string, password: string): Promise<boolean> => {
    try {
      ...
      if (res.ok) {
        ...
        // SEC-642: Check if password change is required (default credentials)
  ...
  const chat = useChat({
    ...
    onDashboardAuthExpired: handleDashboardAuthExpired,
    ...
    (async () => {
      try {
        ...
        if (data.authenticated) {
          // SEC-642: If default password is in use, force password change
          if (data.requirePasswordChange) {
            setRequirePasswordChange(true);
            setAuthState('authenticated');
            return;
          }
          // Daemon epoch check — detect daemon restart and clear stale chat caches.
          // Without this, localStorage has messages from previous daemon lifetimes
          // that flash before cursor_reset clears them (the "flip-flop" bug).
          if (data.daemonEpoch) {
            const storedEpoch = localStorage.getItem('shizuha_daemon_epoch');
            if (storedEpoch && storedEpoch !== data.daemonEpoch) {
              // Daemon restarted — clear all chat caches and cursors
              const keysToRemove: string[] = [];
              for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
                if (key?.startsWith('shizuha_chat_') || key?.startsWith('shizuha_cursor_')) {
                  keysToRemove.push(key);
                }
              }
              keysToRemove.forEach(k => localStorage.removeItem(k));
              if (keysToRemove.length > 0) {
                console.log(`[Dashboard] Daemon epoch changed — cleared ${keysToRemove.length} stale cache entries`);
              }
            }
            localStorage.setItem('shizuha_daemon_epoch', data.daemonEpoch);
          }
          setAuthState('authenticated');
          return;
        }
  ...
  useEffect(() => {
    ...
    window.fetch = async (...args) => {
      ...
      // SEC-642: Intercept 403 with requirePasswordChange flag
      if (res.status === 403) {
        try {
        ...
        } catch { /* not JSON or no requirePasswordChange — fall through */ }
      }
      if (res.status === 401) {
  ...
  const handleLogout = useCallback(async () => {
    ...
    // Wipe the IndexedDB chat cache + persistent send outbox so the next
    // user on the same browser starts clean. Both are per-browser, not
    // per-account, so cross-account leakage would be real.
    try {
      localStorage.removeItem('shizuha_connect_outbox_v1');
      localStorage.removeItem('shizuha_connect_cursor_v1');
      const { clearAllCachedMessages } = await import('./lib/connect-cache');
  ...
  const [syncPending, setSyncPending] = useState<Array<{ agent_id: string; field: string; new_value: unknown; old_value: unknown; updated_by: string }>>([]);
  const [syncConflicts, setSyncConflicts] = useState<Array<{ username: string; reason: string }>>([]);
  ...
  const handleSelectAgent = useCallback((agent: Agent) => {
    ...
    // Set markdown context for workspace image path rewriting
    import('./lib/markdown.js').then((m) => m.setMarkdownAgentContext(agent.username));
  ...
  // Tab guard — only one dashboard tab allowed at a time
  if (tabGuard.isActive === false) {
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-zinc-950">
        <div className="flex flex-col items-center gap-6 text-center px-6 max-w-md">
          <div className="w-16 h-16 rounded-2xl bg-shizuha-600/20 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-shizuha-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25A2.25 2.25 0 0 1 5.25 3h13.5A2.25 2.25 0 0 1 21 5.25Z" />
            </svg>
          </div>
          <div>
            <h2 className="text-lg font-semibold text-zinc-200">Shizuha is open in another tab</h2>
            <p className="text-sm text-zinc-500 mt-2">
              Only one dashboard tab can be active at a time to keep your chat in sync.
            </p>
          </div>
          <button
            onClick={tabGuard.claim}
            className="px-6 py-2.5 bg-shizuha-600 hover:bg-shizuha-500 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer"
          >
            Use here
          </button>
        </div>
  ...
  // Loading
  if (authState === 'loading' || tabGuard.isActive === null) {
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-zinc-950">
  ...
  // SEC-642: Force password change before allowing dashboard access
  if (requirePasswordChange) {
    return (
      <ForcePasswordChange
        onPasswordChanged={() => {
          setRequirePasswordChange(false);
          // Session was invalidated by password change — reload to re-login with new password
          // (ForcePasswordChange already updated the stored credentials in localStorage)
  ...
  return (
    <div className="h-[100dvh] flex bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      {syncConflicts.length > 0 && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-red-900/95 border-b border-red-700 px-4 py-2 text-sm text-red-100">
          <strong>Platform Link Blocked</strong> — {syncConflicts.length} agent conflict(s):
          {syncConflicts.map((c, i) => (
            <span key={i} className="ml-2 font-mono text-red-300">@{c.username}</span>
          ))}
          <span className="ml-2 text-red-400">Resolve via Settings → Agents</span>
          <button onClick={() => setSyncConflicts([])} className="ml-4 text-red-400 hover:text-red-200 cursor-pointer">×</button>
        </div>
      )}
      {syncPending.length > 0 && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-blue-900/95 border-b border-blue-700 px-4 py-2 text-sm text-blue-100 flex items-center">
          <span>
            <strong>Platform Update</strong> — {syncPending.length} pending change(s) from platform
          </span>
          <div className="ml-auto flex gap-2">
            <button onClick={async () => {
              for (const u of syncPending) {
                await fetch('/v1/sync/resolve', { method: 'POST', headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ agent_id: u.agent_id, field: u.field, action: 'accept' }) });
              }
              setSyncPending([]);
            }} className="px-2 py-0.5 bg-blue-700 hover:bg-blue-600 rounded text-xs cursor-pointer">Accept All</button>
            <button onClick={async () => {
              for (const u of syncPending) {
                await fetch('/v1/sync/resolve', { method: 'POST', headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ agent_id: u.agent_id, field: u.field, action: 'reject' }) });
              }
              setSyncPending([]);
            }} className="px-2 py-0.5 bg-zinc-700 hover:bg-zinc-600 rounded text-xs cursor-pointer">Reject All</button>
            <button onClick={() => setSyncPending([])} className="text-blue-400 hover:text-blue-200 cursor-pointer">×</button>
          </div>
        </div>
      )}
            ...
            {selectedAgent && (
              <div className="flex bg-zinc-800 rounded-lg p-0.5 mr-1">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${activeView === 'chat' ? 'bg-zinc-700 text-zinc-200' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  Chat
                </button>
                <button
                  onClick={() => setActiveView('activity')}
                  className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${activeView === 'activity' ? 'bg-zinc-700 text-zinc-200' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  Activity
                </button>
              </div>
