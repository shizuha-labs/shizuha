...
export const agentSchema = z.object({
  ...
  // SCLI-218: compaction target for dynamic-window aliases (cortex/auto).
  // 'planning' (default) = current best-known window; 'conservative' = the
  // alias's advertised context floor, so the session fits every ladder rung.
  compactionWindowMode: z.enum(['planning', 'conservative']).optional(),
  temperature: z.number().min(0).max(2).default(0),
  // 32000 → 32768 (2026-08-09): align with the DeepSeek profile ceiling so
  // the gateway's request budget doesn't silently under-cut the profile by
  // 768 tokens; providers still clamp to their own per-model caps.
  maxOutputTokens: z.number().int().min(100).default(32768),
...
export const toolSearchSchema = z.object({
  /** 'on' = defer MCP schemas behind ToolSearch (default), 'auto' = threshold-based compatibility mode, 'off' = register all upfront */
  mode: z.enum(['auto', 'on', 'off']).default('on'),
...
/**
 * SCLI-20(c): tunable thresholds for the runtime loop-detector
 * (src/agent/loop-detector.ts). Field names mirror LoopDetectorConfig so the
 * parsed section can be passed straight into `new LoopDetector(...)`.
 * Cross-field validation enforces that the hard-stop threshold is never lower
 * than its corresponding warning threshold (otherwise a loop would "break"
 * before it could ever "warn", which is almost certainly a misconfiguration).
 */
export const loopDetectorSchema = z.object({
  /** Consecutive identical tool calls before a soft warning (default 3). */
  warningThreshold: z.number().int().min(1).default(3),
  /** Consecutive identical tool calls before a hard stop (default 5). */
  breakThreshold: z.number().int().min(1).default(5),
  /** Consecutive inline bash probes (no write) before a coaching nudge (default 5). */
  probeLoopWarning: z.number().int().min(1).default(5),
  /** Consecutive inline bash probes (no write) before a hard stop (default 8). */
  probeLoopBreak: z.number().int().min(1).default(8),
}).superRefine((v, ctx) => {
  if (v.breakThreshold < v.warningThreshold) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['breakThreshold'],
      message: `breakThreshold (${v.breakThreshold}) must be >= warningThreshold (${v.warningThreshold})`,
    });
  }
  if (v.probeLoopBreak < v.probeLoopWarning) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['probeLoopBreak'],
      message: `probeLoopBreak (${v.probeLoopBreak}) must be >= probeLoopWarning (${v.probeLoopWarning})`,
    });
  }
});
...
export const perAgentConfigSchema = z.object({
  ...
  maxContextTokens: z.number().int().min(1000).optional(),
  compactionWindowMode: z.enum(['planning', 'conservative']).optional(), // SCLI-218
