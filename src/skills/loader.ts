...
export function loadSkills(cwd: string, options: LoadSkillsOptions = {}): Skill[] {
  ...
  // Bundled integration skills used to be registered here via a hardcoded
  // fallback array. That's now redundant: the daemon's syncBundledSkills()
  // copies every bundled skill (SKILL.md with proper frontmatter) into
  // ~/.shizuha/skills/, where the disk scan above picks them up naturally.
  // Frontmatter in the SKILL.md is the single source of truth for
  // descriptions/tags.
...
function loadSkillsFromDir(dir: string, source: SkillSource): Skill[] {
  ...
  let entries: string[];
  try {
    // PLAT-4189: sort for a DETERMINISTIC load order. `fs.readdirSync` returns
    // raw filesystem order, which differs between two mounts/pods of the same
    // skills content. Skill load order feeds the system-prompt skill catalog
    // (registry insertion order), so an unsorted read makes the rebuilt system
    // prompt reorder across a pod restart — invalidating the entire vLLM prefix
    // cache (system prompt + full session history) and forcing an 80-110s
    // re-prefill on the agent's first post-restart turn. A stable sort makes the
    // rebuilt prompt byte-identical so restarts are KV-cache-transparent.
...
function parseSkillFile(
  ...
  try {
    ...
    return {
      ...
      roles: parseAllowedTools(frontmatter.roles),
