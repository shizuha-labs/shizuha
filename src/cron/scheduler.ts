...
export class CronScheduler {
  ...
  private async tick(): Promise<void> {
    try {
      ...
      for (const job of dueJobs) {
        if (await this.store.claimJob(job.id)) {
  ...
  private enqueueJob(jobId: string): void {
    ...
    const isWakeup = job.kind === 'wakeup';
    this.submitToInbox({
      id: `cron-${jobId}-${Date.now()}`,
      channelId: job.deliver.channelId,
      channelType: (job.deliver.channelType ?? 'http') as ChannelType,
      threadId: isWakeup ? job.deliver.threadId : `cron-${jobId}`,
      userId: isWakeup ? 'wakeup-scheduler' : 'cron-scheduler',
      userName: isWakeup ? 'ScheduleWakeup' : 'Cron',
      content: job.prompt,
      timestamp: Date.now(),
      source: 'cron',
      cronJobId: jobId,
      cronJobName: job.name,
      metadata: isWakeup ? { selfInvocation: true, loop: job.loop } : undefined,
    });
  }
}
