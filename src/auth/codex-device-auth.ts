...
export async function codexDeviceAuth(callbacks: DeviceAuthCallbacks): Promise<string> {
  // Step 1: Request device code (with retry on 429)
  let codeResp: Response | undefined;
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (codeResp.status === 429) {
      const delay = Math.min(10_000 * (attempt + 1), 30_000);
      console.warn(`[device-auth] Rate limited (429), retrying in ${delay / 1000}s...`);
  ...
  if (!codeResp || !codeResp.ok) {
    const status = codeResp?.status ?? 0;
    const text = await codeResp?.text().catch(() => '') ?? '';
    if (status === 429) {
      throw new Error('Rate limited by OpenAI. Please wait a few minutes and try again.');
    }
    const short = text.startsWith('<!') ? '(HTML error page)' : text.slice(0, 100);
    throw new Error(`Failed to request device code: ${status} ${short}`);
  }
