...
export function useSpinner(
  ...
  // A caller may slow the spinner down rather than stop it. Stopping is what
  // made a long model call look frozen; slowing keeps the "still alive" signal
  // at a cost the render loop is already paying (the elapsed timer re-renders
  // once a second regardless).
  const tickMs = Math.max(16, intervalMs ?? spinner.interval);
  ...
  }, [animate, spinner.frames.length, tickMs]);
