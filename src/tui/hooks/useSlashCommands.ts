...
import { modelRequiresThinkingForTools } from '../../provider/model-profile.js';
import { buildAgentsMdBoilerplate } from '../utils/projectCommands.js';
...
interface SlashCommandContext {
  ...
  /** Returns true only when a durable session was renamed. */
  ...
  loginShizuha?: (username: string, password: string) => Promise<{
    username: string;
...
const HELP_FULL_TEXT = `Slash Commands:
  ...
  /watch <task|description> [--policy <policy>]
                            Watch a background task; policies: done_only, state_changes, silent
  /watch list               List active watches
  /watch remove <watch|target>
...
export async function handleSlashCommandAsync(input: string, ctx: SlashCommandContext): Promise<SlashCommandResult> {
  ...
  if (cmd === '/login') {
    ...
    try {
      ...
      const providerStatus = result.providerReloaded === false
        ? 'Provider auth reload pending.'
        : 'Provider auth refreshed.';
      if (result.mcpReloaded) {
        return { handled: true, message: `Logged in as ${result.username}. ${providerStatus} MCP auth reloaded.` };
      }
      return {
        handled: true,
        message: `Logged in as ${result.username}. ${providerStatus} MCP auth reload pending: ${result.reloadError ?? 'unknown error'}`,
      };
    } catch (err) {
  ...
  if (cmd === '/compact') {
    try {
      const message = await ctx.compact(arg || undefined);
      return { handled: true, message: message ?? 'Context compacted.' };
    } catch (err) {
      return { handled: true, message: `Compaction failed: ${(err as Error).message}` };
    }
  }

  // SCLI-387: full doctor path including selected-model reachability (async).
  if (cmd === '/doctor') {
    const docCwd = ctx.cwd ?? process.cwd();
    const selectedModel = ctx.getSessionInfo?.()?.model;
...
export function handleSlashCommand(input: string, ctx: SlashCommandContext): SlashCommandResult {
  ...
  switch (cmd) {
    ...
    case '/think': {
      ...
      let level = arg.toLowerCase();
      // SCLI-54: GLM glm47 tool parser requires thinking ON — refuse off.
      const modelSlug = ctx.getSessionInfo?.().model;
      if (level === 'off' && modelSlug && modelRequiresThinkingForTools(modelSlug)) {
        if (ctx.setThinking) ctx.setThinking('on');
        return {
          handled: true,
          message: `Thinking: on (required for ${modelSlug} tool-call parser — /think off is ignored)`,
        };
      }
      if (ctx.setThinking) {
        ctx.setThinking(level);
      }
      return { handled: true, message: `Thinking: ${level}` };
    }
    ...
    case '/doctor':
      // Async path lives in handleSlashCommandAsync (selected-model probe).
      return {
        handled: true,
        message: 'Run /doctor via the interactive session (async diagnostics).',
      };
    ...
    case '/watch':
      return handleWatchCommand(arg, ctx);

    case '/rename': {
      // SCLI-390: never claim success unless a durable session was renamed.
      // Fresh zero-turn TUIs have no sessionId yet — match /fork's truthful reject.
      if (!arg) {
        return { handled: true, message: 'Usage: /rename <name>' };
      }
      if (ctx.renameSession) {
        const renamed = ctx.renameSession(arg);
        if (renamed) {
          return { handled: true, message: `Session renamed to "${arg}"` };
        }
        return { handled: true, message: 'No active session to rename' };
      }
      return { handled: true, message: 'Rename not available' };
    }
    ...
    case '/init': {
      ...
      // SCLI-391: detect project ecosystem — never invent npm for non-Node trees.
      const boilerplate = buildAgentsMdBoilerplate(cwd);
      try {
    ...
    case '/help':
      if (['all', 'full', 'verbose'].includes(arg.trim().toLowerCase())) {
        // SCLI-383: full help in the pager (q/Esc exit, no composer leak) rather
        // than a status-line dump that left the draft active under "any key".
        if (ctx.showInPager) {
          ctx.showInPager(HELP_FULL_TEXT);
          return { handled: true };
        }
...
function handleWatchCommand(arg: string, ctx: SlashCommandContext): SlashCommandResult {
  const registry = ctx.getTaskRegistry?.();
  if (!registry) {
    return { handled: true, message: '/watch requires an active task registry' };
  }

  const trimmed = arg.trim();
  if (!trimmed) {
    return { handled: true, message: 'Usage: /watch <task_id|description> [--policy done_only|state_changes|silent]\n       /watch list\n       /watch remove <watch_id|target>' };
  }

  const [sub, ...restParts] = trimmed.split(/\s+/);
  if (sub?.toLowerCase() === 'list') {
    const watches = registry.listWatches();
    if (watches.length === 0) return { handled: true, message: 'No active watches.' };
    return {
      handled: true,
      message: watches.map((w) => `${w.id}: ${w.target} (${w.policy})`).join('\n'),
    };
  }

  if (sub?.toLowerCase() === 'remove' || sub?.toLowerCase() === 'rm' || sub?.toLowerCase() === 'unwatch') {
    const target = restParts.join(' ').trim();
    if (!target) return { handled: true, message: 'Usage: /watch remove <watch_id|target>' };
    const removed = registry.unwatch(target);
    ctx.onWatchesChanged?.();
    return { handled: true, message: removed ? `Removed watch: ${target}` : `Watch not found: ${target}` };
  }

  const { target, policy } = parseWatchArgs(trimmed);
  if (!target) {
    return { handled: true, message: 'Usage: /watch <task_id|description> [--policy done_only|state_changes|silent]' };
  }
  try {
    const { watch, matched } = registry.watch(target, policy);
    ctx.onWatchesChanged?.();
    const matchedText = matched.length > 0
      ? `Matched ${matched.map((t) => t.id).join(', ')}.`
      : 'No current task matched; future matching tasks will inherit this policy.';
    return { handled: true, message: `Watching ${watch.target} (${watch.policy}) as ${watch.id}. ${matchedText}` };
  } catch (err) {
    return { handled: true, message: `Watch failed: ${(err as Error).message}` };
  }
}

function parseWatchArgs(input: string): { target: string; policy: TaskNotifyPolicy } {
  let policy: TaskNotifyPolicy = 'done_only';
  const parts = input.split(/\s+/);
  const filtered: string[] = [];
  for (let i = 0; i < parts.length; i++) {
    const part = parts[i]!;
    if (part === '--policy' || part === '-p') {
      const value = parts[++i] as TaskNotifyPolicy | undefined;
      if (value === 'done_only' || value === 'state_changes' || value === 'silent') {
        policy = value;
      }
      continue;
    }
    const inline = part.match(/^--policy=(done_only|state_changes|silent)$/);
    if (inline) {
      policy = inline[1] as TaskNotifyPolicy;
      continue;
    }
    filtered.push(part);
  }
  return { target: filtered.join(' ').trim(), policy };
}
...
function handleAuthCommand(arg: string, ctx: SlashCommandContext): SlashCommandResult {
  const trimmed = arg.trim();
  ...
  if (!['anthropic', 'openai', 'google', 'cortex'].includes(provider)) {
    return { handled: true, message: `Unknown provider: ${provider}. Use: anthropic, openai, google, cortex` };
  }

  if (!value) {
    const hint = provider === 'cortex'
      ? `Usage: /config auth cortex sk-cortex-…  (or /auth cortex …). Get a key at cortex.shizuha.com → API Keys, or sign in with /login <email> <password>.`
      : `Usage: /config auth ${provider} <token/key>`;
    return { handled: true, message: hint };
  }

  if (provider === 'cortex') {
    // Save the Cortex key AND activate it live (reinit providers + select a cortex
    // model) so the user can resend immediately — no restart needed.
    if (ctx.configureAuth) {
      ctx.configureAuth('cortex', 'cortex/DeepSeek-V4-Flash', value);
      return { handled: true, message: `Cortex key saved and activated. Resend your message.` };
    }
    setCortexApiKey(value);
    return { handled: true, message: `Cortex key saved. Restart or /model to activate.` };
