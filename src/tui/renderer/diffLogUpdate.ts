  ...
  const render = (str: string): boolean => {
    ...
    if (prevRenderedCount === 0) {
      ...
      if (nextVisible > realRows) {
        ...
        if (growth > 0) {
          // ── Content grew: append actual new lines when possible ──
          // If the output is append-only, writing the newly added lines with
          // natural newlines lets the terminal/tmux preserve those lines in
          // scrollback. Scrolling blank lines and then rewriting the tail keeps
          // the screen correct but loses hidden new content from scrollback.
          const appendOnly = prevLines
            .slice(0, prevTotalVisible)
            .every((line, idx) => nextLines[idx] === line);
          ...
          if (appendOnly) {
            for (let idx = prevTotalVisible; idx < nextVisible; idx++) {
              buf.push('\n');
              buf.push('\r');
              buf.push(nextLines[idx]!);
              buf.push('\x1b[K');
            }
          } else {
            // Non-append tall transition, such as replacing the welcome screen
            // with a large completed command output. Clear the previous
            // dynamic frame before writing the replacement naturally; otherwise
            // stale input/status lines get pushed into tmux scrollback above
            // the final answer.
            if (prevBottom > 0) {
              buf.push(`\x1b[${prevBottom}A`);
            }
            if (prevRenderedCount > 0) {
              buf.push(`\x1b[${prevRenderedCount}M`);
            }
            buf.push('\x1b[J');
            for (let row = 0; row < nextVisible; row++) {
              buf.push('\r');
              buf.push(nextLines[row]!);
              buf.push('\x1b[K');
              if (row < nextVisible - 1) {
                buf.push('\n');
              }
            }
          }
