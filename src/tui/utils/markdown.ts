...
// SCLI-243: inline code uses the theme accent (pi mdCode) instead of raw cyan.
const CODE_ON = '\x1b[38;2;138;190;183m';
...
export function renderMarkdown(text: string, width = 80): string {
  ...
  marked.use(
    markedTerminal({
      ...
      // SCLI-243: pi's markdown roles — warm headings, teal inline code,
      // soft-blue links, green code blocks (theme.ts is the single source).
      // marked-terminal's published types under-type these as booleans, but
      // the library documents chalk-style functions — hence the cast.
      heading: chalk.hex(theme.heading).bold,
      firstHeading: chalk.hex(theme.heading).bold,
      codespan: chalk.hex(theme.accent),
      code: chalk.hex(theme.codeBlock),
      link: chalk.hex(theme.link),
      href: chalk.hex(theme.link).underline,
      blockquote: chalk.hex(theme.muted).italic,
      hr: chalk.hex(theme.dim),
    } as unknown as Parameters<typeof markedTerminal>[0]) as ReturnType<typeof markedTerminal>,
  );
