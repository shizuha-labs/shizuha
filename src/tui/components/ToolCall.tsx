...
import { ACTIVE_SPINNER_LIMIT_MS } from './ThinkingIndicator.js';
...
/**
 * SCLI-385: after this many ms a running tool card escalates from bare
 * spinner/timer to an actionable stall banner. Matches ThinkingIndicator's
 * long-run threshold so the TUI has one slow-work contract.
 */
export const TOOL_STALL_THRESHOLD_MS = ACTIVE_SPINNER_LIMIT_MS;

export function resolveToolRunningPresentation(elapsedMs: number): {
  showStallBanner: boolean;
  stallMessage: string | null;
} {
  if (elapsedMs < TOOL_STALL_THRESHOLD_MS) {
    return { showStallBanner: false, stallMessage: null };
  }
  return {
    showStallBanner: true,
    // Keep the copy professional and actionable (operator 2026-08-05 UX bar):
    // still working, how long, how to stop — not an internal "no progress" scare.
    stallMessage: 'Taking longer than expected · Esc to interrupt',
  };
}
...
/** Running tool chrome: spinner + duration + optional stall banner (SCLI-385). */
const RunningToolChrome: React.FC<{
  entry: ToolCallEntry;
  commandPreview: string | null;
  inlineSummary: string;
}> = ({ entry, commandPreview, inlineSummary }) => {
  const { elapsedMs, formatted } = useElapsedTime(true, entry.id);
  const presentation = resolveToolRunningPresentation(elapsedMs);
  const { frame } = useSpinner(
    'dots9',
    true,
    presentation.showStallBanner ? 1_000 : undefined,
  );
  const accent = presentation.showStallBanner ? 'yellow' : palette.tool;

  return (
    <Box flexDirection="column">
      <Box>
        <Text color={accent}>{frame} </Text>
        <Text bold color={palette.tool}>{getToolIcon(entry.name)} {getDisplayToolName(entry.name)}</Text>
      ...
      {presentation.showStallBanner && presentation.stallMessage ? (
        <Box marginLeft={3}>
          <Text color="yellow">{presentation.stallMessage}</Text>
        </Box>
      ) : null}
    </Box>
  );
...
export const ToolCall: React.FC<ToolCallProps> = ({ entry, cwd, verbosity = 'normal' }) => {
  ...
  return (
    <Box flexDirection="column" marginTop={0} marginBottom={0}>
      <Box>
        <Text color={entry.isError ? palette.error : palette.success}>{entry.isError ? '\u2717' : '\u2713'} </Text>
        <Text bold color={palette.tool}>{getToolIcon(entry.name)} {getDisplayToolName(entry.name)}</Text>
        {commandPreview ? <Text dimColor> {commandPreview}</Text> : inlineSummary ? <Text dimColor> {inlineSummary}</Text> : null}
      ...
      {entry.result && entry.isError && (
        <Box marginLeft={3}>
          <Text color={palette.error}>{cwd ? wrapFileLinks(resultText, cwd) : resultText}</Text>
        </Box>
      )}
      {isHiddenResult && (
        <Box marginLeft={3}>
