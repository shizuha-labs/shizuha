...
export const MultiLineInput: React.FC<MultiLineInputProps> = ({
  ...
  inputHandlerRef.current = (input: string, key: any) => {
    ...
    // Ctrl+J is LF (0x0A) on a real terminal/tmux. Ink may expose it either as
    // ctrl+j or as a bare newline with key.return=true, so this must run before
    // the Enter submit branch. Enter's standard terminal encoding is CR (0x0D).
    ...
    // Submit path: Enter / Ctrl+M are CR. LF is reserved for the advertised
    // Ctrl+J multiline contract above.
    const isSubmit = key.return || input === '\r' || (key.ctrl && input.toLowerCase() === 'm');
    if (isSubmit) {
    ...
    // Ctrl+Backspace / Ctrl+W / Alt+Backspace: delete previous word run.
    //
    // Terminals overwhelmingly send DEL (\u007f) for plain Backspace and BS
    // (\u0008) for Ctrl+Backspace — xterm, gnome-terminal, iTerm2, VS Code.
    // \u0008 was missing here, so Ctrl+Backspace fell through to the held-run
    // handler below and deleted a SINGLE character: holding it crawled through
    // the line one letter at a time instead of one word at a time (operator
    // 2026-08-04). Alt+Backspace (\u001b\u007f), the other conventional
    // word-delete, was unhandled too.
    //
    // A held Ctrl+Backspace can arrive coalesced as several BS bytes in one
    // input event, so delete one word per byte rather than one word per event.
    const ctrlBackspaceRun = /^\u0008+$/.test(input) ? input.length : 0;
    ...
    if (isCtrlWordDelete) {
      if (currentCursor > 0) {
        ...
        for (let n = 0; n < Math.max(1, ctrlBackspaceRun) && start > 0; n++) {
          start = findPreviousWordStart(currentValue, start);
        }
    ...
    // Backspace: remove grapheme cluster LEFT of cursor.
    // Ink names terminal Backspace (ASCII DEL 0x7f) as key.delete; stableUseInput
    // remaps that to key.backspace so this path is the one that fires. Do not
    // require !key.delete here in a way that depends on Ink's raw name.
    if (key.backspace && !key.delete) {
      const del = applyBackwardDelete(currentValue, currentCursor);
      if (del) commitValue(del.text, del.cursor);
      return;
    }

    // Forward Delete (CSI 3~ only): remove grapheme cluster AT/under the cursor.
    // Must NOT fire for ASCII DEL — that is Backspace and is a no-op at EOL
    // if routed here (operator 2026-08-15: shizuha1 Backspace did not clear).
    if (key.delete && !key.backspace) {
      const del = applyForwardDelete(currentValue, currentCursor);
      if (del) commitValue(del.text, del.cursor);
      return;
    }
  ...
  const rowLimit = Math.max(1, maxRows ?? visualLines.length);
  const safeCursorVisualIndex = Math.max(0, cursorVisualIndex);
  const maxWindowStart = Math.max(0, visualLines.length - rowLimit);
  const windowStart = Math.min(
    maxWindowStart,
    Math.max(0, safeCursorVisualIndex - Math.floor(rowLimit / 2)),
  );
  const visibleLines = visualLines.slice(windowStart, windowStart + rowLimit);
  ...
  return (
    <Box flexDirection="column" width={targetWidth}>
      {visibleLines.map((line, visibleIndex) => {
        ...
        const prefix = i === 0 ? `${promptChar} ` : (visibleIndex === 0 && windowStart > 0 ? '… ' : '  ');
        const trailing = ' '.repeat(Math.max(0, rightGutter));
        return (
