...
export const ApprovalDialog: React.FC<ApprovalDialogProps> = ({ request, onResolve, queueSize }) => {
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="yellow" paddingX={1} marginY={0}>
      <Box>
        <Text bold color={palette.warning}>{'\u26A0'} Permission Required</Text>
      ...
      <Box marginTop={1}>
        <Text>
          <Text bold color={palette.success}>[Y]</Text><Text>es  </Text>
          <Text bold color={palette.error}>[N]</Text><Text>o  </Text>
          <Text bold color={palette.accent}>[A]</Text><Text>lways allow this tool</Text>
        </Text>
      </Box>
    </Box>
  );
