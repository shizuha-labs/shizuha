...
interface InputBoxProps {
  ...
  /**
   * External draft seed (SCLI-382). The editor owns live keystrokes; App only
   * snapshots through onDraftChange so a key does not re-render the whole root.
   */
  draftValue?: string;
  draftVersion?: number;
...
export const InputBox: React.FC<InputBoxProps> = React.memo(({
  ...
  const [internalValue, setInternalValue] = useState(() => draftValue ?? '');
  ...
  useEffect(() => {
    ...
    onDraftChange?.(next);
    // draftVersion is the explicit external-reset signal. draftValue is also a
    // dependency so pager remounts receive the latest snapshot.
  }, [draftValue, draftVersion, onDraftChange]);
  ...
  inputBoxHandlerRef.current = (_input: string, key: any) => {
    if (key.meta && key.upArrow && queuedCount > 0 && onDequeueQueuedPrompts) {
      ...
      if (prompts.length > 0) {
        setValue((prev) => [prev, ...prompts].map((part) => part.trim()).filter(Boolean).join('\n\n'));
        setHistoryIndex(-1);
        setCompletions(null);
      }
    } else if (key.escape) {
  ...
  // Do not invite queueing during a provider no-header wait —
  // that copy implied useful progress and encouraged lossy input (SCLI-371).
  const activePlaceholder = isStalled
    ? 'Provider still waiting — Esc to cancel · /model to switch (queue paused)'
    : isProcessing
  ...
  return (
    <Box flexDirection="column" width={frameWidth}>
      {(isStalled || queuedCount > 0) && (
        <Box flexDirection="column" paddingX={1} marginBottom={1}>
          {isStalled && (
            <Text color={palette.warning}>
              {stalledGlyph} Long provider wait {formatStallDuration(stalledMs)}
          ...
          {queuedCount > 0 && (
            <Text dimColor>{queuedGlyph} Queued! Alt+Up to edit all queued messages</Text>
          )}
        </Box>
      )}
      ...
      {completions && completions.length > 1 && (
        <Box gap={2} marginBottom={0}>
          {completions.map((c) => (
            <Text key={c} color={palette.accent}>{c}</Text>
          ))}
        </Box>
      )}
