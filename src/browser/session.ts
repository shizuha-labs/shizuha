...
export class BrowserSession {
  ...
  private _humanSession: HumanBrowserSession | null = null;

  constructor(onClose?: () => void, mode: BrowserMode = 'fast') {
    this.onClose = onClose;
    this._mode = mode;
  }

  get mode(): BrowserMode { return this._mode; }

  /** Get the human session (for mouse/keyboard tool access). Only available in human mode. */
  get humanSession(): HumanBrowserSession | null { return this._humanSession; }
  ...
  async navigate(url: string): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).navigate(url);
  ...
  async screenshot(): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).screenshot();
  ...
  async click(selector: string): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).click(selector);
  ...
  async type(selector: string, text: string): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).type(selector, text);
  ...
  async scroll(direction: 'up' | 'down'): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).scroll(direction);
  ...
  async getText(selector?: string): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).getText(selector);
    const page = await this.ensurePage();
  ...
  async evaluate(script: string): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).evaluate(script);
  ...
  async back(): Promise<string> {
    if (this._mode === 'human') return (await this.ensureHuman()).back();
    const page = await this.ensurePage();
  ...
  /** Lazy-init human session. */
  private async ensureHuman(): Promise<HumanBrowserSession> {
    if (!this._humanSession) {
      const { HumanBrowserSession: HBS } = await import('./human-session.js');
      this._humanSession = new HBS(this.onClose);
    }
    return this._humanSession;
  }

  async close(): Promise<string> {
    if (this._humanSession) {
      return this._humanSession.close();
    }
