...
export type ChannelType =
  ...
  | 'connect'        // Shizuha Connect — unified messaging (agents as participants)
...
export interface InboundMessage {
  ...
  source?: 'user' | 'cron' | 'inter-agent' | 'heartbeat';
...
export interface Channel {
  ...
  fenceRuntimeRollIngress?(): void;
  resumeRuntimeRollIngress?(): void;
  ...
  /**
   * Report runtime telemetry over channels that support it.
   * Best-effort; callers must not depend on delivery.
   */
...
export interface ConnectChannelConfig {
  type: 'connect';
  /** WebSocket URL to Connect AgentChatConsumer, e.g. ws://shizuha-connect:8019/connect/ws/connect/agent/ */
  url: string;
  /** Agent JWT token for authentication */
  token: string;
  /** Agent UUID */
  agentId?: string;
