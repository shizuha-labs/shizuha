...
type AuthTokenSource = 'broker' | 'agent-token';
...
export class ShizuhaWSChannel implements Channel {
  ...
  private activeTokenSource: AuthTokenSource = 'broker';
  ...
  /** Prevents duplicate fallback attempts from close/error/auth_error races. */
  private fallbackRetryInFlight = false;
  /** Set when ws reports an HTTP auth rejection before close fires. */
  private sawAuthHandshakeRejection = false;
  ...
  private connect(): void {
    if (!this.activeToken) {
      this.fallbackToAgentToken('broker token absent');
      return;
    }
    ...
    const wsUrl = `${this.url}${this.url.includes('?') ? '&' : '?'}token=${encodeURIComponent(this.activeToken)}`;

    logger.info({ url: this.url, agentId: this.agentId, tokenSource: this.activeTokenSource }, 'Connecting to shizuha-agent');
    ...
    socket.on('close', (code: number, reason: any) => {
      ...
      if (generation !== this.connectionGeneration || this.ws !== socket) {
        logger.debug(
          { code, generation, currentGeneration: this.connectionGeneration },
          'Ignoring stale shizuha-agent socket close after replacement connection started',
        );
        return;
      }
      ...
      if (shouldReconnect && this.shouldFallbackAfterAuthRejection()) {
        this.fallbackToAgentToken(`WS handshake rejected (${code}: ${reason.toString()})`);
    ...
    socket.on('error', (err: Error) => {
      ...
      if (this.isAuthRejection(err.message)) {
        this.sawAuthHandshakeRejection = true;
      }
      // The 'close' event will fire after this, triggering reconnect
    });
  }

  private shouldFallbackAfterAuthRejection(): boolean {
    return this.activeTokenSource === 'broker' && this.sawAuthHandshakeRejection;
  }

  private isAuthRejection(message: unknown): boolean {
    return typeof message === 'string' && /(?:401|403|unauthori[sz]ed|forbidden)/i.test(message);
  }

  private async fallbackToAgentToken(reason: string): Promise<void> {
    if (!this.running || this.fallbackRetryInFlight || this.activeTokenSource === 'agent-token') return;
    this.fallbackRetryInFlight = true;
    this.sawAuthHandshakeRejection = false;
    ...
    try {
      ...
      logger.warn({ reason, agentUsername }, 'Falling back to shizuha-id agent token for Connect WS');
      const token = await new AgentTokenManager({ agentUsername, platformUrl }).getToken();
      if (!token) {
        logger.warn({ reason, agentUsername }, 'Agent-token fallback did not produce a token');
        this.scheduleReconnect();
        return;
      }

      this.activeToken = token;
      this.activeTokenSource = 'agent-token';
      this.reconnectAttempt = 0;
      this.connect();
    } catch (err) {
      logger.warn({ err, reason }, 'Agent-token fallback failed');
      this.scheduleReconnect();
    } finally {
      this.fallbackRetryInFlight = false;
    }
  }
  ...
  private handleServerMessage(msg: Record<string, unknown>): void {
    ...
    switch (type) {
      ...
      case 'auth_error':
        ...
        if (this.activeTokenSource === 'broker' && this.isAuthRejection(msg.message)) {
          this.fallbackToAgentToken(String(msg.message));
        }
        break;
      ...
      case 'execute': {
        ...
        const executionId = (msg.execution_id as string) || (msg.session_id as string) || crypto.randomUUID();
