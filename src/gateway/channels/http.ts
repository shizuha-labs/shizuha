...
import { isValidPrompt } from '../../utils/prompt-validation.js';
import { getLastContextBudget } from '../../prompt/builder.js';
...
export interface HttpChannelOptions {
  ...
  /** Exact harness/provider readiness exported to the fleet RuntimeLane probe. */
...
export class HttpChannel implements Channel {
  ...
  fenceRuntimeRollIngress(): void { this.runtimeRollIngressFenced = true; }

  resumeRuntimeRollIngress(): void { this.runtimeRollIngressFenced = false; }
  ...
  private registerRoutes(): void {
    ...
    app.get('/health', async () => ({
      ...
      runtimeRollDrain: this.getRuntimeRollDrain(),
      // EVOL-15: most recent composed-context budget measurement (null until the
    ...
    app.post<{ Body: RuntimeRollDrainRequest }>(
      ...
      async (request, reply) => {
        ...
        if (!this.armRuntimeRollDrain) {
          return reply.code(503).send({ ok: false, error: 'runtime_roll_drain_unavailable' });
        }
        try {
          return this.armRuntimeRollDrain(request.body ?? {} as RuntimeRollDrainRequest);
        } catch (error) {
    ...
    app.post<{ Body: { prompt: string; model?: string; permissionMode?: string } }>(
      ...
      async (request, reply) => {
        ...
        // SCLI-413: require a non-empty STRING prompt. The old `!body?.prompt`
        // admitted whitespace-only strings, numbers, booleans, and objects,
        // which were pushed to the inbox and crossed the agent/provider
        // boundary as autonomous SSE turns.
        if (!isValidPrompt(body?.prompt)) {
          return reply.status(400).send({ error: 'prompt must be a non-empty string' });
        }
        if (this.runtimeRollIngressFenced) {
          return reply.status(503).send({ error: 'runtime_roll_draining', retryable: true });
        }
  ...
  private handleWsMessage(
    ...
    switch (type) {
      ...
      case 'message': {
        ...
        if (this.runtimeRollIngressFenced) {
          try { ws.send(JSON.stringify({ type: 'error', data: { message: 'runtime_roll_draining', retryable: true } })); } catch { /* ignore */ }
          break;
        }
