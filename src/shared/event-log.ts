...
export class EventLog {
  ...
  /**
   * Clear all events for an agent (used on daemon restart / session reset).
   * When agent containers restart, their session context is gone — replaying
   * events from a dead session causes ghost messages in the dashboard.
   */
  clearAgent(agentId: string): number {
    const result = this.db.prepare('DELETE FROM event_log WHERE agent_id = ?').run(agentId);
    return result.changes;
  }

  /**
   * Clear all events (used on daemon startup).
   * Agent containers restart with the daemon — old events reference dead sessions.
   */
  clearAll(): number {
    const result = this.db.prepare('DELETE FROM event_log').run();
    // Reset autoincrement so seq starts from 1
    this.db.prepare("DELETE FROM sqlite_sequence WHERE name = 'event_log'").run();
    return result.changes;
  }
