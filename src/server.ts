...
import { runAgent, registerTelemetryWindow, unregisterTelemetryWindow, createTurnTelemetrySink } from './agent/loop.js';
...
import { isValidPrompt, normalizePrompt } from './utils/prompt-validation.js';
...
export async function startServer(port = 8015, host = '0.0.0.0'): Promise<void> {
  ...
  app.post<{ Body: QueryRequest }>('/v1/query/stream', async (request, reply) => {
    ...
    // SCLI-413: require a non-empty STRING prompt (type + trimmed content).
    // The old `!body?.prompt` admitted whitespace-only strings, numbers,
    // booleans, and objects, which crossed the provider boundary as
    // autonomous SSE turns.
    ...
    // Hoisted so the finally block can deregister even if setup throws.
    let telWin: TurnTelemetryWindow | null = null;
    ...
    try {
      ...
      // Pre-warm discovery before sizing ToolSearch auto mode and prompt budgets.
      ...
      if (toolSearchEnabled && activeSession.messages.length > 0) {
        toolSearchState.markDiscoveredFromHistory(activeSession.messages);
      ...
      // SCLI-74: per-request telemetry window registered in a shared Set so
      // /metrics aggregates across concurrent requests without races. Use a
      // unique runId per request (not session id) to avoid duplicate
      // (runId, turnIndex) pairs when the same session handles multiple requests.
      telWin = new TurnTelemetryWindow();
      const telSink = createTurnTelemetrySink();
      registerTelemetryWindow(telWin);
      const serveRunId = `${activeSession.id}#${crypto.randomUUID().slice(0, 8)}`;
      const serveAgentLabel = (body as { agentLabel?: string }).agentLabel;
      ...
      const compactAutomaticallyIfNeeded = async (reportedPromptTokens = 0): Promise<boolean> => {
        ...
        store.replaceMessages(activeSession.id, compacted.messages);
      ...
      while (!maxTurns || turnIndex < maxTurns) {
        // Resume/nudge paths can add context between completed turns. Enforce
        // the zero-network safety gate immediately before every provider call.
        ...
        // Compact after every completed subturn, including the final text-only
        // answer, so the persisted session is immediately safe to resume.
        await compactAutomaticallyIfNeeded(lastReportedPromptTokens);
      ...
      // Deregister this request's telemetry window so /metrics stops aggregating it.
      if (telWin) unregisterTelemetryWindow(telWin);
    }
  ...
  app.post<{ Body: QueryRequest }>('/v1/query/ndjson', async (request, reply) => {
    const body = request.body;
    // SCLI-413: require a non-empty STRING prompt.
    if (!isValidPrompt(body?.prompt)) {
      return reply.status(400).send({ error: 'prompt must be a non-empty string' });
    }
