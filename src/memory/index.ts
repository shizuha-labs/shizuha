...
export function openAiEmbeddingKeyFromEnv(): string {
  const explicit = process.env['EMBEDDING_API_KEY']?.trim();
  if (explicit) return explicit;

  const openaiKey = process.env['OPENAI_API_KEY']?.trim();
  const cortexKey = process.env['CORTEX_API_KEY']?.trim();
  const vllmKey = process.env['VLLM_API_KEY']?.trim();
  if (!openaiKey) return '';

  // This module calls api.openai.com directly. Fleet Cortex agents expose their
  // Cortex key through OPENAI_API_KEY for OpenAI-compatible chat routing; using
  // that token for OpenAI embeddings creates repeated 401s in every agent log.
  if (openaiKey === cortexKey || openaiKey === vllmKey || openaiKey.startsWith('sk-cortex-')) {
    return '';
  }
...
export class MemoryIndex {
  ...
  constructor(workspace: string, config?: MemoryIndexConfig) {
    ...
    // Priority: explicit config > EMBEDDING_API_KEY > real OpenAI API key > Codex OAuth access_token.
    if (!this.cfg.embeddingApiKey) {
      this.cfg.embeddingApiKey = openAiEmbeddingKeyFromEnv();
