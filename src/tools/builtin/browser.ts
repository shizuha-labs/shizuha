...
export const browserTool: ToolHandler = {
  ...
  parameters: z.object({
    ...
    mode: z.enum(['fast', 'human']).optional().describe(
      'Browser mode: "fast" (default) uses headless Playwright. "human" uses a real browser on a virtual ' +
      'display with uinput mouse/keyboard — undetectable by anti-bot systems. In human mode, click/type ' +
      'by selector and evaluate are disabled; use the mouse and keyboard tools with coordinates instead.',
    ),
  }),
  readOnly: false,
  riskLevel: 'medium',
