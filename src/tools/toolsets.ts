...
const MCP_ALL_SERVICES = [
  ...
  'mcp__inventory*', 'mcp__books*', 'mcp__finance*', 'mcp__connect*',
];
...
export const BUILTIN_TOOLSETS: Record<string, Toolset> = {
  ...
  'coding-agent': {
    name: 'coding-agent',
    description: 'Focused coding agent tools (~14 tools). Matches what agentically-trained models were trained on.',
    include: ['bash', 'read', 'write', 'edit', 'glob', 'grep', 'web_fetch', 'task', 'todo_write', 'todo_read', 'ask_user', 'skill', 'apply_patch', 'enter_plan_mode', 'exit_plan_mode'],
  },
  'social-media': {
    name: 'social-media',
    description: 'Browser automation for social media (X/Twitter). Minimal tool set for local VL models.',
    include: [
  ...
  'security_engineer': {
    ...
    description: 'Security audits, pen testing — read tools + bash + browser. pulse/wiki/drive/id MCP',
    include: [...READ_TOOLS, ...TASK_TOOLS, 'bash', 'browser', ...MCP_COMMON, 'mcp__id*'],
  },
  'technical_writer': {
  ...
  'data_analyst': {
    ...
    include: [
      ...
      ...MCP_COMMON, 'mcp__books*', 'mcp__finance*', 'mcp__inventory*',
    ],
  },
};
