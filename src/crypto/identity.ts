...
export function loadOrCreateAgentKeypair(workspaceDir: string, agentUsername?: string): Keypair & { x25519Public: string; x25519Private: string } {
  const username = agentUsername || process.env['AGENT_USERNAME'] || '';

  // New persistent config path: ~/.shizuha/agents/{username}/identity/agent-keypair.json
  // SHIZUHA_HOME overrides os.homedir() for containers where HOME may not be mounted on the host.
  const home = process.env['SHIZUHA_HOME'] || os.homedir();
  const newIdentityDir = username
    ? path.join(home, '.shizuha', 'agents', username, 'identity')
    : '';
  const newKeypairPath = newIdentityDir
    ? path.join(newIdentityDir, 'agent-keypair.json')
    : '';

  // Old workspace path: {workspaceDir}/.identity/agent-keypair.json
  const oldIdentityDir = path.join(workspaceDir, '.identity');
  const oldKeypairPath = path.join(oldIdentityDir, 'agent-keypair.json');

  // 1. Check new path first
  if (newKeypairPath && fs.existsSync(newKeypairPath)) {
    try {
      const data = JSON.parse(fs.readFileSync(newKeypairPath, 'utf-8'));
      if (data.publicKey && (data.privateKeyPem || data.privateKey) && data.x25519Public && data.x25519Private) {
        return data;
      }
    } catch { /* regenerate */ }
  }

  // 2. Auto-migrate: old path exists but new path doesn't
  if (newKeypairPath && fs.existsSync(oldKeypairPath)) {
    try {
      const data = JSON.parse(fs.readFileSync(oldKeypairPath, 'utf-8'));
      if (data.publicKey && (data.privateKeyPem || data.privateKey) && data.x25519Public && data.x25519Private) {
        // Strip redundant privateKey field if privateKeyPem is available
        const migrated: Record<string, unknown> = { ...data };
        if (migrated.privateKeyPem && migrated.privateKey) {
          delete migrated.privateKey;
        }
        fs.mkdirSync(newIdentityDir, { recursive: true });
        fs.writeFileSync(newKeypairPath, JSON.stringify(migrated, null, 2), { mode: 0o600 });
        console.warn(`[IDENTITY] Migrated agent keypair from workspace to ~/.shizuha/agents/${username}/identity/. The old .identity/ path will be removed in v0.9.0.`);
        return migrated as unknown as Keypair & { x25519Public: string; x25519Private: string };
      }
    } catch { /* fall through to old path or regenerate */ }
  }

  // 3. Fall back to old workspace path (no username available)
  if (!newKeypairPath && fs.existsSync(oldKeypairPath)) {
    try {
      const data = JSON.parse(fs.readFileSync(oldKeypairPath, 'utf-8'));
      if (data.publicKey && (data.privateKeyPem || data.privateKey) && data.x25519Public && data.x25519Private) {
  ...
  // 4. Generate new keypair at the preferred path
  ...
  if (newKeypairPath) {
    fs.mkdirSync(newIdentityDir, { recursive: true });
    fs.writeFileSync(newKeypairPath, JSON.stringify(keypair, null, 2), { mode: 0o600 });
  } else {
    fs.mkdirSync(oldIdentityDir, { recursive: true });
    fs.writeFileSync(oldKeypairPath, JSON.stringify(keypair, null, 2), { mode: 0o600 });
  }
