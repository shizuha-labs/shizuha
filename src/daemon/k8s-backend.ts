/**
 * Local/no-op fleet adapter for Shizuha Code.
 *
 * The Hive/k3s writer lives in Origin `shizuha-labs/hive-runtime`.
 * This module keeps manager/dashboard compiling without shipping that
 * actuator. `isK8sAgent` is always false, so `shizuha up` stays
 * local/docker + the browser dashboard.
 */
