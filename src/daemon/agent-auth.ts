...
// Compromised public key fingerprints (SHA-256 of raw Ed25519 public key hex).
// Keys in this list are permanently rejected by the gateway during challenge exchange.
const COMPROMISED_KEY_FINGERPRINTS = new Set([
  '7c2d75d37f26f143ddb1cfa9e7c06a2cb2eaa291aac6a839871913fbf3a59cce',
]);

function isKeyCompromised(publicKeyHex: string): boolean {
  const fingerprint = crypto.createHash('sha256').update(publicKeyHex).digest('hex');
  return COMPROMISED_KEY_FINGERPRINTS.has(fingerprint) || COMPROMISED_KEY_FINGERPRINTS.has(publicKeyHex);
}
...
function agentIdentityPath(agent: AgentInfo): string {
  const home = process.env['SHIZUHA_HOME'] || process.env['HOME'] || os.homedir();
  const newPath = path.join(home, '.shizuha', 'agents', agent.username, 'identity', 'agent-keypair.json');
  if (fs.existsSync(newPath)) return newPath;
  // Fallback to legacy workspace path
  const oldPath = path.join(home, '.shizuha', 'workspaces', agent.username, '.identity', 'agent-keypair.json');
  if (fs.existsSync(oldPath)) return oldPath;
  return newPath; // Default to new path (will error on read if missing)
}

function loadAgentPublicKey(agent: AgentInfo): string {
  // Check inline keypair in agent config first — set during provisioning and required
  // when the daemon runs as a k3s pod without access to the host ~/.shizuha/agents/ tree.
  const inlineKey = (agent.keypair as { publicKey?: string } | null)?.publicKey;
...
export function issueAgentGatewayChallenge(agent: AgentInfo): AgentGatewayChallenge {
  cleanupExpired();
  const publicKey = loadAgentPublicKey(agent);
  if (isKeyCompromised(publicKey)) {
    throw new Error(`Agent "${agent.username}" is using a compromised public key — rotate the keypair before authenticating`);
  }
