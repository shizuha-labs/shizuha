...
export class AuditLogger {
  ...
  /**
   * Log a structured people-ops action. These entries use the stable
   * `people_ops_action` tool tag so audit_log can query them across sessions.
   */
  logPeopleOpsAction(
    agent: string,
    action: {
    ...
    const primitive = action.primitive.trim();
    const targetAgent = action.targetAgent.trim();
    const entry: AuditEntry = {
      id,
      timestamp: new Date().toISOString(),
      agent,
      tool: 'people_ops_action',
      targetAgent,
      tier,
      primitive,
      inputSummary: summarize({
        ...
        tier,
        primitive,
        pulseTask: action.pulseTask,
        reason: action.reason,
      }),
      resultSummary: action.result ? summarize(action.result, 300) : undefined,
      riskFlags: ['people-ops', tier],
      phase: 'after',
    };
    this.write(entry);
    return id;
  ...
  query(opts: {
    ...
    const { limit = 50, tool, agent, riskOnly, targetAgent, tier, primitive } = opts;
    const normalizedTier = tier?.trim().toUpperCase();
    const normalizedPrimitive = primitive?.trim();
    const normalizedTarget = targetAgent?.trim();
    try {
      const raw = fs.readFileSync(this.logPath, 'utf-8');
      const lines = raw.trim().split('\n').filter(Boolean);
      const entries: AuditEntry[] = [];
      // Read from end for efficiency, but keep scanning until enough matching
      // rows are found. Filtered audit queries (people-ops target/tier/primitive
      // lookups in particular) must cross noisy session boundaries instead of
      // stopping after a fixed tail window.
      for (let i = lines.length - 1; i >= 0 && entries.length < limit; i--) {
        try {
          const entry = JSON.parse(lines[i]!) as AuditEntry;
          if (tool && entry.tool !== tool) continue;
          if (agent && entry.agent !== agent) continue;
          if (normalizedTarget && entry.targetAgent !== normalizedTarget) continue;
          if (normalizedTier && entry.tier !== normalizedTier) continue;
          if (normalizedPrimitive && entry.primitive !== normalizedPrimitive) continue;
