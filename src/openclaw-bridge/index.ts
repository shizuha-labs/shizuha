...
const GATEWAY_PASSWORD = process.env['GATEWAY_PASSWORD'] ?? process.env['SHIZUHA_GATEWAY_SECRET'] ?? (() => {
  const secret = crypto.randomBytes(32).toString('hex');
  console.warn('[openclaw-bridge] WARNING: GATEWAY_PASSWORD not set — using random ephemeral secret. Bridge auth will not persist across restarts.');
  return secret;
})();
...
export class OpenClawBridge {
  ...
  // Telemetry (reported to the platform over Connect — mirrors claude/codex bridges).
  private lastTurnOutputTokens = 0;
  private lastTurnDurationMs = 0;
  private outputAtLastComplete = 0;
  private lastCompleteAt = Date.now();
  ...
  private messageQueue: Array<{ clientId: string; content: string; messageId?: string }> = [];
  /** Connect inbound message id for inject-once ack on turn complete. */
  ...
  /** Telemetry snapshot reported to the platform over Connect (mirrors claude/codex). */
  private buildTelemetry(): Record<string, unknown> {
    ...
    return {
      ...
      context: { used_tokens: null, max_tokens: this.modelMaxTokens(), pct: null },
      usage: {
      ...
      health: { ok: this.recentErrors.length === 0, recent_errors: this.recentErrors.slice(-10) },
    };
  }
  ...
  private startTelemetry(): void {
    ...
    this.emitTelemetry();
    console.log(`[openclaw-bridge] Telemetry enabled (every ${Math.round(ms / 1000)}s -> Connect)`);
  }
    ...
    try {
      ...
      this.connectClient = new ConnectClient({
        ...
        onMessage: (convId, content, senderId, senderName, messageId) => {
          ...
          console.log(`[openclaw-bridge] [Connect] Message from ${senderName} in conv ${convId.substring(0, 8)}… len=${content.length} busy=${!!this.activeThreadId}`);
          // Inject-once: queue/run the turn; ack on complete (never re-require message_user).
          if (this.activeThreadId) {
            this.messageQueue.push({ clientId: connectClientId, content, messageId });
          } else {
            this.startExecution(connectClientId, content, messageId);
          }
        },
        onConfigUpdate: (cfg) => {
          console.log(`[openclaw-bridge] agent_config_update received: keys=${Object.keys(cfg).join(',')}`);
          if (typeof cfg['contextPrompt'] === 'string') {
            this.opts.contextPrompt = cfg['contextPrompt'];
            console.log(`[openclaw-bridge] contextPrompt updated (${cfg['contextPrompt'].length} chars)`);
          }
          const unhandled = Object.keys(cfg).filter(k => k !== 'contextPrompt');
          if (unhandled.length) {
            console.log(`[openclaw-bridge] agent_config_update: keys noted (no live-apply): ${unhandled.join(',')}`);
          }
        },
      });
...
`);
    ...
    if (stream === 'lifecycle') {
      ...
      if (phase === 'end' || phase === 'error') {
        ...
        this.turnCount++;
        {
          ...
          if (phase === 'error') {
            this.recentErrors.push({ ts: _now, level: 'error', msg: String((data?.error as Record<string, unknown>)?.message ?? 'Agent error').slice(0, 300) });
  ...
  private async findOpenClawCli(): Promise<string> {
    ...
    const installCommands = [
      ...
      `npm install -g --prefix ${localPrefix} openclaw`,
    ];
    for (const cmd of installCommands) {
      try {
        execSync(cmd, { encoding: 'utf-8', timeout: 180_000, stdio: ['ignore', 'pipe', 'pipe'] });
        console.log('[openclaw-bridge] OpenClaw CLI installed successfully');
        ...
        if (msg.includes('EACCES') && cmd === installCommands[0]) {
          console.log('[openclaw-bridge] Global install failed (EACCES), trying user-local...');
          continue;
        }
        console.error('[openclaw-bridge] Auto-install failed: ' + msg);
      }
    }
    ...
    try {
      ...
      if (account?.accessToken) {
        ...
        try {
          ...
          if (!payload) throw new Error('OpenAI access token payload missing');
  ...
  private async startServer(): Promise<void> {
    ...
    this.app.get('/health', async () => ({
      status: this.gatewayConnected && this.hasAuth ? 'ok' : 'degraded',
      bridge: 'openclaw',
      model: this.opts.model,
      gatewayConnected: this.gatewayConnected,
      initialized: this.gatewayConnected,
      authenticated: this.hasAuth,
      providerHealthy: this.gatewayConnected && this.hasAuth,
      provider_available: this.gatewayConnected && this.hasAuth,
      quota_ok: true,
      in_backoff: false,
      busy: this.activeThreadId !== null,
      queueDepth: this.messageQueue.length,
  ...
  private async startExecution(clientId: string, content: string, connectMessageId?: string): Promise<void> {
    ...
    this.activeConnectMessageId = isConnect ? (connectMessageId ?? null) : null;
    this.accumulatedContent = '';
  ...
  private processQueue(): void {
    ...
    this.startExecution(next.clientId, next.content, next.messageId);
  }
...
export async function startOpenClawBridge(opts: OpenClawBridgeOptions): Promise<void> {
  // Validate before the startup summary, bridge construction, persistent state,
  // telemetry, signal handlers, or gateway work can produce partial startup.
  const validatedOpts = {
    ...opts,
  ...
  console.log(
    `[openclaw-bridge] Startup summary: ${JSON.stringify({
      agentId: validatedOpts.agentId,
      agentUsername: validatedOpts.agentUsername,
      model: validatedOpts.model,
      contextPrompt: summarizePromptForLog(validatedOpts.contextPrompt),
    })}`,
  );
  if (isBridgePromptDebugEnabled() && validatedOpts.contextPrompt?.trim()) {
    console.log(`[openclaw-bridge] Context prompt begin\n${validatedOpts.contextPrompt}\n[openclaw-bridge] Context prompt end`);
  }

  const bridge = new OpenClawBridge(validatedOpts);
